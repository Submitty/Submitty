...
class TestLoadTriggers(unittest.TestCase):
    def setUp(self):
        ...
        for i in range(0, len(self.data_master)):
            with (migrator.TRIGGERS_PATH / 'master' / 'test_fn_master_{}.sql'.format(i)).open('w') as file:
        ...
        for i in range(0, len(self.data_course)):
            with (migrator.TRIGGERS_PATH / 'course' / 'test_fn_course_{}.sql'.format(i)).open('w') as file:
    ...
    def test_master(self):
        ...
        self.assertEqual(
            'Loading trigger functions to master...\n  {}\nDONE\n'
                .format('\n  '.join(['test_fn_master_{}'.format(self.data_master.index(n)) for n in data])),
            sys.stdout.getvalue()
        )
    ...
    def test_course(self):
        ...
        for i in range(0, num_courses):
            self.assertEqual('submitty_s{0}_c{0}'.format(i), mock_class.call_args_list[i+1][0][0]['dbname'])
        ...
        for coursedb in coursedbs:
            ...
            course_logs += ('Loading trigger functions to s{0}.c{0}...\n  {1}\nDONE\n'
                               .format(i, '\n  '.join(
                                    ['test_fn_course_{}'.format(self.data_course.index(n)) for n in data]
                                )))
            i += 1
