...
class PollUtilsTester extends \PHPUnit\Framework\TestCase {
    ...
    public function testExportDataWithNonEmptyPolls() {
        $polls = [
            new PollModel($this->core, 0, "Poll #1", "Is this the first poll?", "single-response", ["Yes", "No", "Maybe"], [0, 2], "closed", ["bitdiddle" => 0, "aphacker" => 1], "2020-01-11", null, "never", "when_ended"),
            new PollModel($this->core, 1, "Poll #2", "Is this the second poll?", "single-response", ["Yes", "No", "Definitely not"], [0], "open", ["bitdiddle" => 2, "aphacker" => 0], "2020-01-12", null, "always", "never"),
            new PollModel($this->core, 2, "Poll #3", "Is this the fourth poll?", "multilple-response", ["Yes", "No", "Maybe"], [1], "ended", ["bitdiddle" => 1, "aphacker" => 2], "2020-01-13", null, "when_ended", "always"),
        ];
        $expected_data = [
            [
                "id" => 0,
                ...
                "show_correct_answer" => "when_ended"
            ],
            [
                "id" => 1,
                "name" => "Poll #2",
