...
def return_result(score, message, status):
    result = {
        "status": "success",
        "data": {"score": score, "message": message, "status": status},
    }
...
def do_the_grading():
    ...
    if os.path.exists(my_access_file):
        ...
        return_result(
            score=0, message="ERROR!  access file does not exist", status="failure"
        )

    first_access_timestamp = datetime.strptime(
        first_access_timestamp_string, "%Y-%m-%d %H:%M:%S%z"
    )
    ...
    submission_timestamp = datetime.strptime(
        submission_timestamp_string, "%Y-%m-%d %H:%M:%S%z"
    )

    # compute the difference in seconds
    access_duration = int(
        (submission_timestamp - first_access_timestamp).total_seconds()
    )
    ...
    my_message = (
        ...
        + str(active_version)
        + "\nfirst access: "
        + str(first_access_timestamp)
        + "\nsubmission: "
        + str(submission_timestamp)
        + "\nallowed minutes: "
        + str(allowed_minutes)
        + "\nduration: "
        + duration_string
        ...
        + str(penalty_minutes)
        + "\npenalty per minute: "
        + str(penalty_per_minute)
        + "\ndeduction (before rounding): "
        + str(my_points_deducted)
        + "\ndeduction (after rounding): "
        + str(my_score * max_penalty)
    )

    if my_score > 0:
        # red message when there is a penalty
        return_result(score=my_score, message=my_message, status="failure")
    else:
        # blue message when full credit
        return_result(score=my_score, message=my_message, status="information")
