...
describe("Test cases revolving around user profile page", () => {
    ...
    it("Should start with accurate values", () => {
        cy.get('[data-testid="username-row"]').should(
            "contain.text",
            "instructor"
        );
        cy.get('[data-testid="givenname-row"]').should("contain.text", "Quinn");
        cy.get('[data-testid="familyname-row"]').should(
            "contain.text",
            "Instructor"
        );
        cy.get('[data-testid="pronouns-row"]').should(
            "contain.text",
            "She/Her"
        );
        cy.get('[data-testid="email-row"]').should(
            "contain.text",
    ...
    it("Time zone selector should work", () => {
        cy.get('[data-testid="time-zone-dropdown"]').should(
            "contain.text",
            "NOT_SET/NOT_SET"
        );
        cy.get('[data-testid="time-zone-dropdown"]').select(
        ...
        cy.get('[data-testid="popup-message"]')
            .next()
            .should("contain.text", "Time-zone updated successfully");
        cy.get('[data-testid="time-zone-dropdown"]').select("NOT_SET/NOT_SET");
    });

    it("Should error then succeed uploading profile photo", () => {
        const filePath =
            "../more_autograding_examples/image_diff_mirror/submissions/student1.png";
        ...
        cy.get('[data-testid="popup-message"]')
            ...
            .should(
                "contain.text",
                "No image uploaded to update the profile photo"
            );
        cy.get('[data-testid="upload-photo-button"]').click();
        ...
        cy.get('[data-testid="popup-message"]')
            .next()
            .next()
            .should("contain.text", "Profile photo updated successfully!");
    });
    ...
    it("Flagging an innapropriate photo", () => {
        ...
        cy.visit("/user_profile");
        cy.get('[data-flagged="flagged"]').should(
            "contain.text",
            "Your preferred image was flagged as inappropriate."
        );
    ...
    it("Should open and close the popups", () => {
        ...
        testFormOpening(
            "#secondary-email-notify-row",
            "#edit-secondary-email-form"
        );
