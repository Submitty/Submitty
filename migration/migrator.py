...
def parse_args():
    ...
    parser.add_argument('--environment', '-e', dest='environments', choices=ENVIRONMENTS,
...
def handle_migration(args):
    ...
    for environment in args.environments:
        if environment in ['system', 'master']:
            ...
            args.use_environment = args.environment
...
def migrate_environment(connection, environment, args):
    ...
    with connection.cursor() as cursor:
        cursor.execute('SELECT migration, commit_time, status FROM migrations_{} ORDER BY id'.format(environment))
    ...
    if args.direction == 'up':
        ...
        if args.initial is True:
            ...
            # we don't want to actually run any of the other migrations here
            args.fake = True
        for key in keys:
...
def run_migration(connection, migration, environment, args):
    ...
    with connection.cursor() as cursor:
        if migration['db']:
            cursor.execute('UPDATE migrations_{} SET commit_time=CURRENT_TIMESTAMP, status=%d WHERE id=%s'.format(environment), (status, migration['id'],))
        else:
            cursor.execute('INSERT INTO migrations_{} (id, status) VALUES(%s, %d)', (migration['id'], status,))
    connection.commit()


def load_migration_module(name, path):
    spec = importlib.util.spec_from_file_location('module.{}'.format(name), path)
