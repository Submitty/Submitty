    ...
    public function showStatus($gradeable_id) {
        ...
        if (count($total_users) > 0) {
            foreach ($total_users as $key => $value) {
             ...
             /* Allow instructor to see Peer completion stats */
            if ($isPeerGradeable) {
                $sections['peer_stu_grad'] = [
                  ...
                  // 'total_peer_components' =>
                   'total_components' => count($gradeable->getPeerComponents()) * sizeof($num_submitted),
                   // Graded Components = Sum of components graded by each peer grader in a class.
                   'graded_components' => $peer_graded_components,
            ...
            if ($peer) {
                ...
                foreach ($total_users as $key => $value) {
                    ...
                    $sections['all']['ta_graded_components'] += isset($graded_components[$key]) ? $graded_components[$key] : 0;
                }
                $sections['all']['total_components'] -= $num_components;
                $sections['all']['graded_components'] -= $my_grading;
                $sections['all']['ta_graded_components'] -= 0;
                if ($gradeable->isTeamAssignment()) {
