...
def update_worker_json(name, entry):
    ...
    try:
        ...
        autograding_utils.log_message(JOB_ID, message="ERROR: autograding_workers.json entry for {0} is malformed. {1}".format(e, name))
        autograding_utils.log_stack_trace(job_id=JOB_ID, trace=traceback.format_exc())
        return False
    ...
    if host == "localhost":
        try:
            ...
            autograding_utils.log_message(JOB_ID, message="Successfully updated local autograding_TODO/autograding_worker.json")
            return True
        except Exception as e:
            autograding_utils.log_stack_trace(job_id=JOB_ID, trace=traceback.format_exc())
            autograding_utils.log_message(JOB_ID, message="ERROR: could not mv to local autograding_TODO/autograding_worker.json due to the following error: "+str(e))
        ...
        try:
            ...
            autograding_utils.log_stack_trace(job_id=JOB_ID, trace=traceback.format_exc())
            autograding_utils.log_message(JOB_ID, message="ERROR: could not ssh to {0}@{1} due to following error: {2}".format(user, host,str(e)))
        ...
        try:
            ...
            autograding_utils.log_message(JOB_ID, message="Successfully forwarded autograding_worker.json to {0}".format(name))
            success = True
        except Exception as e:
            autograding_utils.log_stack_trace(job_id=JOB_ID, trace=traceback.format_exc())
            autograding_utils.log_message(JOB_ID, message="ERROR: could not sftp to foreign autograding_TODO/autograding_worker.json due to the following error: "+str(e))
...
def prepare_job(my_name,which_machine,which_untrusted,next_directory,next_to_grade):
    ...
    try:
        ...
        autograding_utils.log_stack_trace(job_id=JOB_ID, trace=traceback.format_exc())
        autograding_utils.log_message(JOB_ID, message="ERROR: failed preparing submission zip or accessing next to grade "+str(e))
    ...
    if address == "localhost":
        try:
            ...
            autograding_utils.log_stack_trace(job_id=JOB_ID, trace=traceback.format_exc())
            autograding_utils.log_message(JOB_ID, message="ERROR: could not move files due to the following error: "+str(e))
    ...
    autograding_utils.log_message(JOB_ID, jobname=item_name, which_untrusted=which_untrusted,
...
def unpack_job(which_machine,which_untrusted,next_directory,next_to_grade):
    ...
    if which_machine == "localhost":
        ...
        #TODO catch other types of exception as we identify them.
            ...
            autograding_utils.log_stack_trace(job_id=JOB_ID, trace=traceback.format_exc())
            autograding_utils.log_message(JOB_ID, message="ERROR: Could not retrieve the file from the foreign machine "+str(e))
    ...
    try:
        ...
        autograding_utils.log_stack_trace(job_id=JOB_ID, trace=traceback.format_exc())
        autograding_utils.log_message(JOB_ID,jobname=item_name,message="ERROR: Exception when unpacking zip. For more details, see traces entry.")
    ...
    print(msg)
    autograding_utils.log_message(JOB_ID, jobname=item_name, which_untrusted=which_untrusted, is_batch=is_batch, message=msg)
    return True
...
def grade_queue_file(my_name, which_machine,which_untrusted,queue_file):
    ...
    try:
        ...
        if not prep_job_success:
            ...
            autograding_utils.log_message(JOB_ID, message=str(my_name)+" ERROR unable to prepare job: " + queue_file)
        ...
        autograding_utils.log_message(JOB_ID, message=str(my_name)+" ERROR attempting to grade item: " + queue_file + " exception " + repr(e))
    ...
    try:
        ...
        autograding_utils.log_message(JOB_ID, message=str(my_name)+" ERROR attempting to remove queue file: " + queue_file + " exception=" + str(e))
    try:
        os.remove(grading_file)
    except Exception as e:
        autograding_utils.log_stack_trace(job_id=JOB_ID, trace=traceback.format_exc())
        print (my_name, " ERROR attempting to remove grading file: ", grading_file, " exception=",str(e))
        autograding_utils.log_message(JOB_ID, message=str(my_name)+" ERROR attempting to remove grading file: " + grading_file + " exception=" + str(e))
...
def checkout_vcs_repo(my_file):
    ...
    try:
        ...
        try:
            ...
            # exception on git rev-list
                ...
                autograding_utils.log_message(job_id,message="ERROR: failed to determine version on master branch " + str(error))
                os.chdir(checkout_path)
        ...
        # exception on git clone
            ...
            autograding_utils.log_message(job_id,message="ERROR: failed to clone repository " + str(error))
            os.chdir(checkout_path)
    ...
    # exception in constructing full git repository url/path
        ...
        autograding_utils.log_message(job_id,message="ERROR: failed to construct valid repository url/path" + str(error))
        os.chdir(checkout_path)
...
def get_job(my_name,which_machine,my_capabilities,which_untrusted,overall_lock):
    ...
    if time_delta > datetime.timedelta(milliseconds=100):
        ...
        autograding_utils.log_message(JOB_ID, message=str(my_name)+" WARNING: submitty_autograding shipper get_job time "+str(time_delta))
...
def shipper_process(my_name,my_data,full_address,which_untrusted,overall_lock):
    ...
    while True:
        try:
            ...
            print (my_message)
            autograding_utils.log_message(JOB_ID, message=my_message)
            time.sleep(1)
...
def launch_shippers(worker_status_map):
    ...
    for file_path in Path(INTERACTIVE_QUEUE).glob("GRADING_*"):
        ...
        autograding_utils.log_message(JOB_ID, message="Remove old queue file: " + file_path)
        os.remove(file_path)

    for file_path in Path(SUBMITTY_DATA_DIR, "autograding_TODO").glob("untrusted*"):
        file_path = str(file_path)
        autograding_utils.log_message(JOB_ID, message="Remove autograding TODO file: " + file_path)
        os.remove(file_path)
    for file_path in Path(SUBMITTY_DATA_DIR, "autograding_DONE").glob("*"):
        file_path = str(file_path)
        autograding_utils.log_message(JOB_ID, message="Remove autograding DONE file: " + file_path)
        os.remove(file_path)
    ...
    for name, machine in autograding_workers.items():
        if worker_status_map[name] == False:
            ...
            autograding_utils.log_message(JOB_ID, message="{0} could not be reached, so we are not spinning up shipper threads.".format(name))
            continue
        if 'enabled' in machine and machine['enabled'] == False:
            print("{0} is disabled, so we are not spinning up shipper threads.".format(name))
            autograding_utils.log_message(JOB_ID, message="{0} is disabled, so we are not spinning up shipper threads.")
            continue
        try:
            full_address = ""
            ...
            autograding_utils.log_message(JOB_ID, message="ERROR: autograding_workers.json entry for {0} contains an error: {1} For more details, see trace entry.".format(name,e))
            continue
    ...
    try:
        while True:
            ...
            if alive != total_num_workers:
                autograding_utils.log_message(JOB_ID, message="ERROR: #shippers="+str(total_num_workers)+" != #alive="+str(alive))
