...
def cleanup(test):
    subprocess.call(["rm"] + ["-f"] + glob.glob(os.path.join(test.testcase_path, "data", "*pdf")))
...
def too_few(test):
    ...
    subprocess.call(
        [
            "cp",
            os.path.join(SAMPLE_SUBMISSIONS, "words_249.pdf"),
...
def too_many(test):
    ...
    subprocess.call(
        [
            "cp",
            os.path.join(SAMPLE_SUBMISSIONS, "words_1463.pdf"),
            os.path.join(test.testcase_path, "data"),
        ]
    )
    test.run_compile()
...
def just_right(test):
    ...
    subprocess.call(
        [
            "cp",
            os.path.join(SAMPLE_SUBMISSIONS, "words_881.pdf"),
            os.path.join(test.testcase_path, "data"),
        ]
    )
    test.run_compile()
