...
def grade_queue_file(config, my_name, which_machine, which_untrusted, queue_file):
    ...
    try:
        ...
        if e.args and e.args[0] == "ERROR: the submission directory does not exist":
            config.logger.log_message(
                f"ERROR: submission directory does not exist for {queue_file}. "
                f"Removing broken queue file and skipping job."
            )
            config.logger.log_stack_trace(traceback.format_exc())
            grading_cleanup(config, my_name, queue_file, grading_file)
            return
        ...
        try:
            ...
            if not os.path.isdir(submission_dir):
                config.logger.log_message(
                    f"ERROR: submission directory does not exist: {submission_dir}. "
                    f"Removing broken queue file and skipping job."
                )
                config.logger.log_stack_trace(traceback.format_exc())
                grading_cleanup(config, my_name, queue_file, grading_file)
