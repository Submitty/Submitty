import RubricHeaderScore from '../../vue/src/components/ta_grading/RubricHeaderScore.vue';
...
describe('RubricHeaderScore', () => {
    ...
    it('renders minus sign when totalScore is null (ungraded)', () => {
        cy.mount(RubricHeaderScore, { props: { totalScore: null, maxValue: 10 } });
        cy.get('[data-testid="grading-total"]').should('contain', '\u2212 / 10');
    });

    it('applies no extra class when total=0 and max=0', () => {
        cy.mount(RubricHeaderScore, { props: { totalScore: 0, maxValue: 0 } });
    ...
    it('applies green-background when score exceeds max (extra credit)', () => {
        cy.mount(RubricHeaderScore, { props: { totalScore: 12, maxValue: 10 } });
        cy.get('[data-testid="grading-total"]').should('have.class', 'green-background');
    });

    it('applies red-background when totalScore is 0 but maxValue > 0', () => {
        cy.mount(RubricHeaderScore, { props: { totalScore: 0, maxValue: 10 } });
        cy.get('[data-testid="grading-total"]').should('have.class', 'red-background');
    });
});
