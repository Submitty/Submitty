...
class Container:
    ...
    def create(self, execution_script, arguments, more_than_one):
        ...
        mount = {
            self.directory: {"bind": self.directory, "mode": "rw"},
        ...
        try:
            if self.is_server:
                ...
                container_ulimits = rlimit_utils.build_ulimit_argument(
                    self.container_rlimits, self.image
                )
                command = (
                    [
                        execution_script,
        ...
        self.log_container(
            "CREATED", self.full_name, self.container.short_id, timer() - container_create_time
        )
        client.close()
    ...
    def log_docker_error(self, message):
        ...
        queue_file = os.path.join(
            "/var/local/submitty/autograding_tmp", untrusted, "tmp/TMP_SUBMISSION/queue_file.json"
        )
    ...
    def start(self, logfile):
        ...
        self.log_container(
            "STARTED", self.full_name, self.container.short_id, timer() - container_start_time
        )
    ...
    def cleanup_container(self, logfile):
        ...
        self.log_function(
            f"{dateutils.get_current_time()} docker container {self.container.short_id} destroyed"
        )
        self.log_container(
            "DESTROYED", self.full_name, self.container.short_id, timer() - cleanup_start
        )
...
class ContainerNetwork(secure_execution_environment.SecureExecutionEnvironment):
    ...
    def network_containers_routerless(self, containers, random_str):
        ...
        network = client.networks.create(
            network_name, driver="bridge", ipam=ipam_config, internal=True
        )
        client.close()

        self.log_container("NET_CREATED", network_name, network.short_id)
    ...
    def network_containers_with_router(self, containers, random_str):
        ...
        for container in containers:
            ...
            ipam_config = docker.types.IPAMConfig(pool_configs=[ipam_pool])
            network = client.networks.create(
                network_name, ipam=ipam_config, driver="bridge", internal=True
            )
    ...
    def cleanup_networks(self):
        ...
        for network in self.networks:
            try:
                network.remove()
                self.log_container("NET_DESTROYED", network.name, network.short_id)
                network.client.api.close()
                network.client.close()
                self.log_message(
                    f"{dateutils.get_current_time()} destroying docker network {network}"
                )
            except Exception:
                self.log_message(
        ...
        for container in sorted_networked_containers:
            ...
            for connected_container_name in sorted_connections:
                connected_container = self.get_container_with_name(
                    connected_container_name, containers
                )
                network_name = f"{container.full_name}_network_{random_str}"
                ...
                if router is not None:
                    ...
                    # If this node is not the router, we must inject the router
                        ...
                        network_name = (
                            f"{self.untrusted_user}_{connected_container_name}_network_{random_str}"
                        )
        ...
        for action_obj in self.dispatcher_actions:
            ...
            if action_type == "delay":
                ...
                self.send_message_to_processes(
                    containers, action_obj["string"], action_obj["containers"]
                )
            elif action_type in ["stop", "start", "kill"]:
                self.send_message_to_processes(
                    containers, f"SUBMITTY_SIGNAL:{action_type.upper()}\n", action_obj["containers"]
                )
    ...
    def setup_for_random_output(self, testcase_dependencies):
        ...
        for container in self.solution_containers:
            self._setup_single_directory_for_random_output(
                container.directory, testcase_dependencies
            )
            self._run_pre_commands(container.directory)
        ...
        for container in self.containers:
            if len(self.containers) > 1:
                public_dir = os.path.join(
                    self.tmp_results, "results_public", self.name, container.name
                )
