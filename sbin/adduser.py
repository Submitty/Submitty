...
def main():
    ...
    if user is not None:
        print(
            "User already exists! Hit enter on any question to use existing value for that field."
        )
        defaults = user

    givenname = get_input("User givenname", defaults["user_givenname"])
    preferred = get_input("User preferred name", defaults["user_preferred_givenname"], True)
