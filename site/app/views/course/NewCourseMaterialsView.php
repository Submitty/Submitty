...
class NewCourseMaterialsView extends AbstractView {
    /**
     * @param User $user
    ...
    public function listCourseMaterials(User $user, array $course_materials_db) {
        ...
        foreach ($course_materials as $path => $material) {
            $priorities[$path] = $material->getPriority();
        }
        ...
        uasort($course_materials, $sort_priority); //may need to change

        /*$restored = [];
        //Restore the priorities for each file/folder
        foreach ($priorities as $key => &$priority) {
            $restored[$key] = $priority;
        }
        $priorities = $restored;*/
        ...
        foreach ($course_materials as $course_material) {
            ...
            if ($course_material->isDir()) {
                $directories[$course_material->getPath()] = $course_material->getPriority();
                continue;
            }
