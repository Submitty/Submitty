...
class GradeInquiryController extends AbstractController {
    ...
    public function requestGradeInquiry($gradeable_id) {
        ...
        if (!($graded_gradeable->getSubmitter()->hasUser($user) || $can_inquiry)) {
            return MultiResponse::JsonOnlyResponse(
                JsonResponse::getFailResponse('Insufficient permissions to request grade inquiry')
            );
        }

        try {
            $this->core->getQueries()->insertNewGradeInquiryRequest($graded_gradeable, $user, $content, $gc_id);
    ...
    public function makeGradeInquiryPost($gradeable_id) {
        ...
        if (!$graded_gradeable->hasGradeInquiryRequest()) {
            return MultiResponse::JsonOnlyResponse(
    ...
    public function changeGradeInquiryStatus($gradeable_id) {
        ...
        try {
            ...
            $this->core->getQueries()->saveGradeInquiryRequest($grade_inquiry);
            if ($content != "") {
