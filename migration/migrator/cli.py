...
def parse_args(argv, config_path=None):
    ...
    parser.add_argument(
        ...
        "--environment",
        dest="environments",
        choices=get_all_environments(),
        action="append",
        required=True,
    )
    parser.add_argument(
        "--course", dest="choose_course", nargs=2, metavar=("semester", "course"), default=None
    )
    ...
    sub = subparsers.add_parser("status", help="Get status of migrations for environment")

    sub = subparsers.add_parser("migrate", help="Run migrations")
    sub.add_argument(
        "--single", action="store_true", default=False, dest="single", help="Only run one migration"
    )
    sub.add_argument(
        "--fake",
        action="store_true",
    ...
    sub = subparsers.add_parser("rollback", help="Rollback the previously done migration")
    sub.add_argument(
        "--fake",
        action="store_true",
        default=False,
    ...
    sub = subparsers.add_parser("load_triggers", help="Load master database trigger functions")
