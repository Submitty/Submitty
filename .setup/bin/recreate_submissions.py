...
def submit(
    ...
    """
    gradeable_path = Path(ARG_DATA_DIR, "courses", semester, course, "submissions", gradeable)
    ...
    with open(submission_path / UAAF_NAME, "w") as uaaf:
        uaaf.write("[]")
    UAS["history"].append(
        {"version": highest_version + 1, "time": current_time_str, "who": user, "type": "upload"}
    )
...
def parseArgs():
    ...
    parser.add_argument("semester", help="the semester containing the gradeable to submit to")
    parser.add_argument("course", help="the course containing the gradeable to submit to")
    parser.add_argument("gradeable", help="the gradeable ID to submit to")
    parser.add_argument(
        "submissions", help="directory containing the file structure of submissions to use"
    )
...
def main():
    ...
    def insert_egdata(
        g_id: str, user_id: str, g_version: int, submission_time: str, autograding_complete=False
    ):
        if ARG_VERBOSE:
            print("    insert_egdata()")
            print(f"    g_id                 = {g_id}")
    ...
    def update_egver(g_id: str, user_id: str, active_version: int):
        ...
        query = select(table_egver).where(
            (table_egver.c.g_id == g_id) & (table_egver.c.user_id == user_id)
        )
        ...
        if len(rows) == 0:  # INSERT
            query = insert(table_egver).values(
                g_id=g_id, user_id=user_id, active_version=active_version
            )
        else:  # UPDATE
            query = (
                update(table_egver)
                ...
                .where(
                    (table_egver.columns.g_id == g_id) & (table_egver.columns.user_id == user_id)
                )
            )
        if ARG_VERBOSE:
            print(f"    SQL: {query}")
    ...
    for user in os.listdir(ARG_SUBMISSIONS):
        ...
        versions = [int(f) for f in user_dirlist if Path(ARG_SUBMISSIONS, user, f).is_dir()]
    ...
    gradeable_path = Path(
        ARG_DATA_DIR, "courses", ARG_SEMESTER, ARG_COURSE, "submissions", ARG_GRADEABLE
    )
