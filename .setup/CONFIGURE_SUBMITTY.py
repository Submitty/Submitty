...
parser = argparse.ArgumentParser(
    description="Submitty configuration script",
...
parser.add_argument(
    "--data-dir",
    default="/var/local/submitty",
    help="Set the data directory for Submitty",
)
parser.add_argument(
    "--websocket-port", default=8443, type=int, help="Port to use for websocket"
)
...
if not os.path.isdir(SUBMITTY_INSTALL_DIR) or not os.access(
    ...
    raise SystemExit(
        "Install directory {} does not exist or is not accessible".format(
            SUBMITTY_INSTALL_DIR
        )
    )

SUBMITTY_DATA_DIR = args.data_dir
if not os.path.isdir(SUBMITTY_DATA_DIR) or not os.access(
    SUBMITTY_DATA_DIR, os.R_OK | os.W_OK
):
    raise SystemExit(
        "Data directory {} does not exist or is not accessible".format(
            SUBMITTY_DATA_DIR
        )
    )
...
for i in range(1, NUM_UNTRUSTED):
    ...
    if uid != FIRST_UNTRUSTED_UID + i:
        raise SystemExit(
            "CONFIGURATION ERROR: untrusted UID not sequential: " + untrusted_user
        )
    elif gid != FIRST_UNTRUSTED_GID + i:
        raise SystemExit(
            "CONFIGURATION ERROR: untrusted GID not sequential: " + untrusted_user
        )
...
defaults = {
    ...
    "sys_admin_email": "",
    "sys_admin_url": "",
    "ldap_options": {"url": "", "uid": "", "bind_dn": ""},
    "saml_options": {"name": "", "username_attribute": ""},
}
...
if not args.worker:
    if "authentication_method" in loaded_defaults:
        loaded_defaults["authentication_method"] = (
            authentication_methods.index(loaded_defaults["authentication_method"]) + 1
        )
...
if args.worker:
    SUPERVISOR_USER = get_input(
        "What is the id for your submitty user?", defaults["supervisor_user"]
    )
    ...
    if not os.path.isdir(DATABASE_HOST):
        DATABASE_PORT = int(
            get_input("What is the database port?", defaults["database_port"])
        )
        print()
    else:
        DATABASE_PORT = defaults["database_port"]

    DATABASE_USER = get_input(
        "What is the global database user/role?", defaults["database_user"]
    )
    print()
    ...
    DATABASE_PASS = get_input(
        "What is the password for the global database user/role {}? {}".format(
            DATABASE_USER, default
        )
    )
    if (
        DATABASE_PASS == ""
        and DATABASE_USER == defaults["database_user"]
        and "database_password" in defaults
    ):
        DATABASE_PASS = defaults["database_password"]
    print()

    DATABASE_COURSE_USER = get_input(
        "What is the course database user/role?", defaults["database_course_user"]
    )
    print()

    default = ""
    if (
        "database_course_password" in defaults
        and DATABASE_COURSE_USER == defaults["database_course_user"]
    ):
        default = "(Leave blank to use same password)"
    DATABASE_COURSE_PASSWORD = get_input(
        "What is the password for the course database user/role {}? {}".format(
            DATABASE_COURSE_USER, default
        )
    )
    if (
        DATABASE_COURSE_PASSWORD == ""
        and DATABASE_COURSE_USER == defaults["database_course_user"]
        and "database_course_password" in defaults
    ):
        DATABASE_COURSE_PASSWORD = defaults["database_course_password"]
    print()

    TIMEZONE = get_input(
        "What timezone should Submitty use? (for a full list of supported timezones see http://php.net/manual/en/timezones.php)",
        defaults["timezone"],
    )
    print()

    DEFAULT_LOCALE = get_input(
        "What default language should the Submitty site use?", "en_US"
    )
    print()

    SUBMISSION_URL = get_input(
        "What is the url for submission? (ex: http://192.168.56.101 or "
        "https://submitty.cs.rpi.edu)",
        defaults["submission_url"],
    ).rstrip("/")
    print()
    ...
    if INSTITUTION_NAME == "" or INSTITUTION_NAME.isspace():
        ...
        INSTITUTION_HOMEPAGE = get_input(
            "What is the url of your institution's homepage? "
    ...
    SYS_ADMIN_EMAIL = get_input(
        "What is the email for system administration?", defaults["sys_admin_email"]
    )
    SYS_ADMIN_URL = get_input(
        "Where to report problems with Submitty (url for help link)?",
    ...
    while True:
        try:
            auth = (
                int(get_input("Enter number?", defaults["authentication_method"])) - 1
            )
        except ValueError:
            auth = -1
    ...
    if AUTHENTICATION_METHOD == "LdapAuthentication":
        ...
        LDAP_OPTIONS["bind_dn"] = get_input(
            "Enter LDAP bind_dn?", LDAP_OPTIONS["bind_dn"]
        )
    ...
    if AUTHENTICATION_METHOD == "SamlAuthentication":
        SAML_OPTIONS["name"] = get_input(
            "Enter name you would like shown to user for authentication?",
            SAML_OPTIONS["name"],
        )
        SAML_OPTIONS["username_attribute"] = get_input(
            "Enter SAML username attribute?", SAML_OPTIONS["username_attribute"]
        )

    CGI_URL = SUBMISSION_URL + "/cgi-bin"

    SUBMITTY_ADMIN_USERNAME = get_input(
        "What is the submitty admin username (optional)?",
        defaults["submitty_admin_username"],
    )

    while True:
        is_email_enabled = get_input(
            "Will Submitty use email notifications? [y/n]", "y"
        )
        if is_email_enabled.lower() in ["yes", "y"]:
            EMAIL_ENABLED = True
            EMAIL_USER = get_input("What is the email user?", defaults["email_user"])
            EMAIL_PASSWORD = get_input(
                "What is the email password", defaults["email_password"]
            )
            EMAIL_SENDER = get_input(
                "What is the email sender address (the address that will appear in the From: line)?",
                defaults["email_sender"],
            )
            EMAIL_REPLY_TO = get_input(
                "What is the email reply to address?", defaults["email_reply_to"]
            )
            EMAIL_SERVER_HOSTNAME = get_input(
                "What is the email server hostname?", defaults["email_server_hostname"]
            )
            try:
                EMAIL_SERVER_PORT = int(
                    get_input(
                        "What is the email server port?", defaults["email_server_port"]
                    )
                )
            except ValueError:
                EMAIL_SERVER_PORT = defaults["email_server_port"]
            EMAIL_INTERNAL_DOMAIN = get_input(
                "What is the internal email address format?",
                defaults["email_internal_domain"],
            )
            break
...
if not args.worker:
    for full_file_name, file_name in [
        (WORKERS_JSON, "autograding_workers.json"),
...
if not args.worker:
    if not os.path.isfile(WORKERS_JSON):
        ...
        vagrant_workers_json = os.path.join(
            SUBMITTY_REPOSITORY, ".vagrant", "workers.json"
        )
...
with open(SUBMITTY_USERS_JSON, "w") as json_file:
    json.dump(config, json_file, indent=2)
shutil.chown(
    SUBMITTY_USERS_JSON, "root", DAEMON_GROUP if args.worker else DAEMONPHP_GROUP
)
