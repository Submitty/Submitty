...
def log_stack_trace(log_path, job_id="UNKNOWN", is_batch=False, which_untrusted="", jobname="", timelabel="", elapsed_time=-1, trace=""):
    ...
    if elapsed_time == "":
        elapsed_time = -1
    elapsed_time_string = "" if elapsed_time < 0 else f'{time:9.3f}'.format(elapsed_time)
    time_unit = "" if elapsed_time < 0 else "sec"
    parts = (easy_to_read_date, f"{job_id:>6s}", f"{batch_string:>5s}", f"{which_untrusted:>11s}",
...
def setup_for_validation(working_directory, complete_config, is_vcs, testcases, job_id, log_path, stack_trace_log_path):
    ...
    pattern_copy("submission_to_validation",
                 patterns['submission_to_validation'], submission_path, tmp_work, tmp_logs)
    if is_vcs:
        checkout_subdir_path = os.path.join(tmp_submission, 'checkout', checkout_subdirectory)
        pattern_copy("checkout_to_validation",
                     patterns['submission_to_validation'], checkout_subdir_path, tmp_work, tmp_logs)

    for c in testcases:
        if c.type == 'Compilation':
            pattern_copy("compilation_to_validation",
                         patterns['compilation_to_validation'], c.secure_environment.directory, tmp_work, tmp_logs)

    # Copy expected files into the tmp_work_test_output path
    test_output_path = os.path.join(tmp_autograding, 'test_output')
    copy_contents_into(job_id, test_output_path, tmp_work_test_output,
                       tmp_logs, log_path, stack_trace_log_path)
    generated_output_path = os.path.join(tmp_autograding, 'generated_output')
    copy_contents_into(job_id, generated_output_path, tmp_work_generated_output,
                       tmp_logs, log_path, stack_trace_log_path)

    # Copy in instructor solution code.
    instructor_solution = os.path.join(tmp_autograding, 'instructor_solution')
    copy_contents_into(job_id, instructor_solution, tmp_work_instructor_solution,
                       tmp_logs, log_path, stack_trace_log_path)
    ...
    copy_contents_into(job_id, custom_validation_code_path, tmp_work,
                       tmp_logs, log_path, stack_trace_log_path)
    ...
    add_permissions_recursive(tmp_work,
                              ...
                              stat.S_IROTH | stat.S_IWOTH | stat.S_IXOTH,
                              stat.S_IROTH | stat.S_IWOTH | stat.S_IXOTH)
    add_permissions(my_runner, stat.S_IXUSR | stat.S_IXGRP |
                    stat.S_IROTH | stat.S_IWOTH | stat.S_IXOTH)
...
def add_all_permissions(path):
    ...
    if os.path.isdir(path):
        ...
        add_permissions(path, stat.S_IRUSR | stat.S_IWUSR | stat.S_IXUSR | stat.S_IRGRP |
...
def lock_down_folder_permissions(top_dir):
    ...
    os.chmod(top_dir, os.stat(top_dir).st_mode & ~stat.S_IRGRP & ~stat.S_IWGRP &
...
def prepare_directory_for_autograding(working_directory, user_id_of_runner, autograding_zip_file, submission_zip_file, is_test_environment, log_path, stack_trace_log_path, SUBMITTY_INSTALL_DIR):
    ...
    old_containers = subprocess.check_output(
        ['docker', 'ps', '-aq', '-f', 'name={0}'.format(user_id_of_runner)]).split()
    ...
    old_networks = subprocess.check_output(
        ['docker', 'network', 'ls', '-qf', 'name={0}'.format(user_id_of_runner)]).split()
    if len(old_containers) > 0:
    ...
    copy_contents_into(job_id, test_input_path, tmp_work_test_input,
                       tmp_logs, log_path, stack_trace_log_path)
...
def archive_autograding_results(config, working_directory, job_id, which_untrusted, is_batch_job, complete_config_obj,
    ...
    if "generate_output" not in queue_obj:
        partial_path = os.path.join(queue_obj["gradeable"],
                                    queue_obj["who"], str(queue_obj["version"]))
        item_name = os.path.join(queue_obj["semester"],
                                 queue_obj["course"], "submissions", partial_path)
    ...
    pattern_copy("work_to_details", patterns['work_to_details'],
                 tmp_work, results_details_dir, tmp_logs)

    # Copy work to public
    if 'work_to_public' in patterns:
        pattern_copy("work_to_public", patterns['work_to_public'],
                     tmp_work, results_public_dir, tmp_logs)

    if os.path.exists(random_output_path):
        pattern_copy("work_to_random_output", [os.path.join(
            random_output_path, 'test*', '**', '*.txt'), ], tmp_work, tmp_results, tmp_logs)
    ...
    if "generate_output" not in queue_obj:
        try:
            ...
            log_message(log_path, job_id, is_batch_job, which_untrusted,
                        item_name, message="ERROR: grade.txt does not exist")
            log_stack_trace(stack_trace_log_path, job_id, is_batch_job,
                            which_untrusted, item_name, trace=traceback.format_exc())
        ...
        try:
            ...
            with open(os.path.join(tmp_logs, "overall.txt"), 'a') as f:
                print("\n\nERROR: Grading incomplete -- Could not open ",
                      os.path.join(tmp_work, "grade.txt"))
                log_message(job_id, is_batch_job, which_untrusted, item_name,
                            message="ERROR: grade.txt does not exist")
                log_stack_trace(job_id, is_batch_job, which_untrusted,
                                item_name, trace=traceback.format_exc())
        ...
        try:
            ...
            with open(os.path.join(tmp_logs, "overall.txt"), 'a') as f:
                ...
                log_message(log_path, job_id, is_batch_job, which_untrusted,
                            item_name, message="ERROR: results.json read/write error")
                log_stack_trace(stack_trace_log_path, job_id, is_batch_job,
                                which_untrusted, item_name, trace=traceback.format_exc())

        # Rescue custom validator files
        custom_validator_output_directory = os.path.join(tmp_results, "custom_validator_output")
        pattern_copy("rescue_custom_validator_validation_jsons", [os.path.join(
            tmp_work, 'validation_results_*.json'), ], tmp_work, custom_validator_output_directory, tmp_logs)
        pattern_copy("rescue_custom_validator_logs", [os.path.join(
            tmp_work, 'validation_logfile_*.txt'), ], tmp_work, custom_validator_output_directory, tmp_logs)
        pattern_copy("rescue_custom_validator_errors", [os.path.join(
            tmp_work, 'validation_stderr_*.txt'), ], tmp_work, custom_validator_output_directory, tmp_logs)
        ...
        log_message(log_path, job_id, is_batch_job, which_untrusted,
                    item_name, "grade:", gradingtime, grade_result)
...
def copy_contents_into(job_id, source, target, tmp_logs, log_path, stack_trace_log_path):
    if not os.path.isdir(target):
        log_message(
            log_path, job_id, message="ERROR: Could not copy contents. The target directory does not exist " + target)
        ...
        for item in os.listdir(source):
            if os.path.isdir(os.path.join(source, item)):
                if os.path.isdir(os.path.join(target, item)):
                    # recurse
                    copy_contents_into(job_id, os.path.join(source, item), os.path.join(
                        target, item), tmp_logs, log_path, stack_trace_log_path)
                elif os.path.isfile(os.path.join(target, item)):
                    log_message(
                        log_path, job_id, message="ERROR: the target subpath is a file not a directory '" + os.path.join(target, item) + "'")
                    raise RuntimeError(
                        "ERROR: the target subpath is a file not a directory '", os.path.join(target, item), "'")
                else:
                    # copy entire subtree
                ...
                try:
                    ...
                    grade_items_logging.log_stack_trace(
                        stack_trace_log_path, job_id=job_id, trace=traceback.format_exc())
                    return
    else:
    ...
    # Separate ** and * for simplicity.
        ...
        for file in files:
            try:
                ...
                traceback.print_exc()
                log_message(
                    log_path, job_id, message="Pre Command could not perform copy: {0} -> {1}".format(file, target))
    else:
        ...
        for file_source in files:
            ...
            try:
                ...
                traceback.print_exc()
                log_message(
                    log_path, job_id, message="Pre Command could not perform copy: {0} -> {1}".format(file_source, file_target))
