...
SYSTEMCTL_WRAPPER_SCRIPT = path.join(
    path.dirname(path.realpath(__file__)), "shipper_utils", "systemctl_wrapper.py"
)
...
def get_log(worker: str, usr: str, url: str) -> None:
    try:
        (target_conn, interm_conn) = ssh_proxy_jump.ssh_connection_allowing_proxy_jump(usr, url)
...
if __name__ == "__main__":
    ...
    subprocess.call(
        ["python3", SYSTEMCTL_WRAPPER_SCRIPT, "stop", "--daemon", "shipper", "--target", "primary"]
    )

    print("Stopping all local worker daemon...")
    subprocess.call(
        ["python3", SYSTEMCTL_WRAPPER_SCRIPT, "stop", "--daemon", "worker", "--target", "primary"]
    )
    ...
    subprocess.call(
        ["python3", SYSTEMCTL_WRAPPER_SCRIPT, "start", "--daemon", "shipper", "--target", "primary"]
    )

    print("Starting the local worker daemon...")
    subprocess.call(
        ["python3", SYSTEMCTL_WRAPPER_SCRIPT, "start", "--daemon", "worker", "--target", "primary"]
    )
    ...
    print("Delaying {0} seconds to allow the system to stablize...".format(delay_in_seconds))
    time.sleep(delay_in_seconds)
    ...
    result = subprocess.run(
        ["su", "-", "submitty_daemon", "-c", cmd], stdout=subprocess.PIPE, universal_newlines=True
    )
    print(result.stdout)
    ...
    if "is inactive" in result.stdout:
        ...
        USER_CONFIG = path.join(
            path.dirname(path.realpath(__file__)), "..", "config", "submitty_users.json"
        )
        WORKER_CONFIG = path.join(
            path.dirname(path.realpath(__file__)), "..", "config", "autograding_workers.json"
        )
