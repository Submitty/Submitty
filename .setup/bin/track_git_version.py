...
if __name__ == "__main__":
  ...
  rainbow_dir = "/usr/local/submitty/GIT_CHECKOUT/RainbowGrades"
  output_dict = {}
  ...
  try:
    ...
    current_commit_hash_rb = subprocess.check_output(['git', 'rev-parse', 'HEAD'], cwd=rainbow_dir)
    current_commit_hash_rb = current_commit_hash_rb.decode('ascii').strip()
    current_short_commit_hash_rb = subprocess.check_output(['git', 'rev-parse', '--short', 'HEAD'], cwd=rainbow_dir)
  ...
  try:
    ...
    current_git_tag_rb = subprocess.check_output(['git', 'describe', '--tag', '--abbrev=0'], cwd=rainbow_dir)
