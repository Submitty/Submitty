...
def add_args(parser):
    parser.add_argument(
        "-n", "--num", default=1, type=int, help="Number of worker machines to configure"
    )
    parser.add_argument(
        "--ip-range",
        default=f"192.168.{random.randint(100, 200)}.0/24",
...
def update_bootstrap(workers):
    ...
    with TemporaryDirectory() as tmpdir:
        ...
        w1 = subprocess.run(["sudo", "mv", filepath, "/etc/bootptab"], check=False)
        if w1.returncode != 0:
            print("\033[31mFailed to save configuration\033[0m")
            sys.exit(1)
        w2 = subprocess.run(
            ["sudo", "launchctl", "kickstart", "-k", "system/com.apple.bootpd"], check=False
        )
        if w2.returncode in [113, 1]:
            if w2.returncode == 1:
                subprocess.run(
                    ["sudo", "launchctl", "bootout", "system/com.apple.bootpd"], check=False
                )
            w3 = subprocess.run(
...
def run(args):
    ...
    action_provision = glob.glob(
        os.path.join(rootdir, ".vagrant", "machines", "ubuntu*", "*", "action_provision")
    )
    if not action_provision:
        ...
        for worker_name in (existing_config if version == 1 else existing_config["workers"]):
            if (
                len(
                    glob.glob(
                        os.path.join(
                            rootdir, ".vagrant", "machines", worker_name, "*", "action_provision"
                        )
                    )
                )
                > 0
                ...
                if (
                    input(
                        "\033[93mWarning: There are existing worker machines that may conflict"
                        " with new configuration.\n"
                        "They can be removed safely with 'vagrant workers destroy'.\n"
                        "Are you sure you would like to proceed without removing them?"
                        " [y/N]\033[0m "
                    )
                    .lower()
                    .strip()
    ...
    for i in range(1, args.num + 1):
        ip = next(ips, None)
        while ip is not None and (
            ip.is_reserved or isinstance(ip, ipaddress.IPv4Address) and str(ip).endswith(".1")
        ):
            ip = next(ips, None)
