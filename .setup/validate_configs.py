...
parser = argparse.ArgumentParser(
    description="Submitty config validation script",
    formatter_class=argparse.ArgumentDefaultsHelpFormatter,
)
parser.add_argument("--worker", action="store_true", default=False, help="Validate worker configs")
parser.add_argument(
    "--install-dir", default="/usr/local/submitty", help="The install directory for Submitty"
)
parser.add_argument(
    "--data-dir", default="/var/local/submitty", help="Set the data directory for Submitty"
)
...
if args.worker:
    CONFIG_REPOSITORY = os.path.join(
        SUBMITTY_INSTALL_DIR, "GIT_CHECKOUT/Submitty/.setup/data/configs/worker"
    )

os.makedirs(SUBMITTY_DATA_DIR, exist_ok=True)

if not os.path.isdir(SUBMITTY_INSTALL_DIR) or not os.access(
    SUBMITTY_INSTALL_DIR, os.R_OK | os.W_OK
):
    raise SystemExit(
    ...
    if os.path.isfile(source_path):
        with open(source_path, "r") as f1, open(destination_path, "r") as f2:
            try:
                ...
                if len(difference) > 0:
                    raise KeyError(
                        f"Required key(s) {difference} not present in {destination_path}."
                    )
