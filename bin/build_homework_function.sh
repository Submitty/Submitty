...
function build_homework {
    ...
    test_input_dir="${course_dir}/test_input/${assignment}"
    test_output_dir="${course_dir}/test_output/${assignment}"
    ...
    log_file="${hw_build_path}/build_log.txt"
    ...
    find "$instructor_solution_dir" -type d -exec chmod -f ug+rwx,g+s,o= {} \;
    find "$instructor_solution_dir" -type f -exec chmod -f ug+rw,o= {} \;

    # Debug: Check if directories exist and are accessible
    if [ ! -d "${course_dir}" ]; then
       echo "Directory ${course_dir} does not exist or is not accessible."
       exit 1
    fi

    if [ ! -d "${instructor_solution_dir}" ]; then
       echo "Directory ${instructor_solution_dir} does not exist or is not accessible."
       exit 1
    fi
    ...
    instructor_solution_compile_command= sed 's://.*$::' ${config_file} | jq -r '.instructor_solution_compile_command' >> $log_file
    command=$(head -n 1 log_file.txt)
#
#    echo $instructor_solution_compile_command >> log_file
#    if [ -z "${instructor_solution_compile_command}" ]; then
#        echo "Error: instructor_solution_compile_command not found or empty in config.json"
#        exit 1
#    fi

    # Create instructor_CMakeLists.txt if cpp files exist in instructor_solution directory
#    any_cpp_files=false
#    first_cpp_file=false
#    for file in "${instructor_solution_dir}"/*; do
#       if [ -f "${file}" ] && [[ "${file}" == *.cpp ]]; then
#           any_cpp_files=true
#           if [ "${first_cpp_file}" = false ]; then
#               echo "Creating ${instructor_cmake_file}..." >> $log_file
#               echo "set(custom_files" > "${instructor_cmake_file}"
#               first_cpp_file=true
#           fi
#           echo "  ${file}" >> "${instructor_cmake_file}"
#       fi
#    done
#
#    if [ "${any_cpp_files}" = true ]; then
#       echo ")" >> "${instructor_cmake_file}"
#       echo "foreach(custom_file \${custom_files})" >> "${instructor_cmake_file}"
#       echo "    get_filename_component(executable_name \${custom_file} NAME_WE)" >> "${instructor_cmake_file}"
#       echo "" >> "${instructor_cmake_file}"
#       echo "    add_executable(\${executable_name} \${custom_file})" >> "${instructor_cmake_file}"
#       echo "" >> "${instructor_cmake_file}"
#       echo "endforeach()" >> "${instructor_cmake_file}"
#       echo "Custom CMakeLists.txt created."
#    else
#       echo "No .cpp files found in ${instructor_solution_dir}."
#    fi


    # Set permissions for the generated executables
    ...
    for file in "${instructor_solution_dir}"/*; do
        if [ -f "${file}" ] && [[ "${file}" == *.cpp ]]; then
            ...
            compile_command=sed "$command "${file} -o ${executable}"
    ...
    for executable_file in "${instructor_solution_executable_dir}"/*; do
        if [[ -x "${executable_file}" ]]; then  # Check if it's executable or not
            ...
            for input_file in "${test_input_dir}"/*.txt; do
                ...
                output_file="${test_output_dir}/${executable_base}_${input_filename%.txt}.out"
