...
def grade_queue_file(config, my_name, which_machine, which_untrusted, queue_file):
    ...
    try:
        ...
        if e.args and e.args[0] == "ERROR: the submission directory does not exist":
            msg = f"ERROR: submission directory does not exist for {queue_file}. Removing broken queue file and skipping job."
            ...
            parts = (easy_to_read_date, f"{JOB_ID:>6s}", f"{'':>5s}", f"{'':>11s}", f"{queue_file:75s}", f"{'':6s} {'':>9s} {'':>3s}", msg)
            autograding_utils.write_to_log(os.path.join(config.logger.log_dir, "missing_submissions.txt"), ' | '.join((str(x) for x in parts)))
        ...
        try:
            ...
            if not os.path.isdir(submission_dir):
                msg = f"ERROR: submission directory does not exist: {submission_dir}. Removing broken queue file and skipping job."
                config.logger.log_message(msg)
                config.logger.log_stack_trace(traceback.format_exc())
