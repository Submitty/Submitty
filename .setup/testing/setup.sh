...
python3 ${COMMAND} ${CURRENT_DIR}/validate_configs.py --install-dir $SUBMITTY_INSTALL_DIR
...
tmp=$(mktemp) && jq --argjson v true '.user_create_account = $v' "$SUBMITTY_INSTALL_DIR/config/submitty.json" > "$tmp" && mv "$tmp" "$SUBMITTY_INSTALL_DIR/config/submitty.json"
