...
const SubmitAndCheckMessage = (gradeable_type, upload_file1,invalid_late_day,valid_late_day = '') => {    
    ...
    if ( gradeable_type === 'non_team') {
        cy.visit(['blank', 'gradeable', gradeable]);
    }
    else {
        cy.visit(['blank', 'gradeable', team_gradeable]);
    }
    ...
    cy.waitPageChange(() => {
        cy.wait(1000); //we need to wait here because if the submission is made exactly at the moment the
                       // the deadline is past due, it will be flaky
        cy.get('#submit').click();
...
describe('Test warning messages for non team gradeable', () => {
    ...
    it('should show a warning message before late submission', () => {
        ...
        cy.login('instructor');
        cy.visit(['blank', 'gradeable',gradeable,'update?nav_tab=5']);        
        cy.get('#date_due')
            .should('exist')
            .clear()
            .type(getCurrentTime())
...
describe('Test warning messages for team gradeable', () => {
    ...
    it('should create team gradeable for testing', () => {
        ...
        cy.logout();
        cy.login('student');
        cy.visit(['blank', 'gradeable',team_gradeable,'team']); 
      ...
      it('should show a confirmation message for the first submission', () => {
       ...
       //The first submission will be done 3 days after the due date and use 2 valid late days for each team member
        ...
        cy.visit(['blank', 'gradeable',team_gradeable,'update?nav_tab=5']); 
        cy.get('#date_due')
            .should('exist')
            .clear()
            .type(getCurrentTime('threeDaysAgo'))
