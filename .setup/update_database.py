...
for course in os.scandir(os.path.join(settings['submitty_data_dir'], "courses", semester)):
    ...
    os.system("""PGPASSWORD='{}' psql --host={} --username={} --dbname={} -c 'CREATE TABLE "attachment" ("attach_id" serial NOT NULL, "post_id" int NOT NULL, CONSTRAINT attachment_pk PRIMARY KEY ("attach_id"))'""".format(settings['database_password'], settings['database_host'], settings['database_user'], db))
    ...
    os.system("""PGPASSWORD='{}' psql --host={} --username={} --dbname={} -c 'ALTER TABLE "attachment" ADD CONSTRAINT "attachment_fk0" FOREIGN KEY ("post_id") REFERENCES "posts"("id")'""".format(settings['database_password'], settings['database_host'], settings['database_user'], db))
