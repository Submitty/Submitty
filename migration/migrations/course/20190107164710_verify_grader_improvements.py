def up(config, conn, semester, course):
    with conn.cursor() as cursor:
        cursor.execute("ALTER TABLE gradeable_component_data ADD COLUMN IF NOT EXISTS gcd_verifier_id VARCHAR(255)")
        cursor.execute("ALTER TABLE gradeable_component_data ADD COLUMN IF NOT EXISTS gcd_verify_time TIMESTAMP")

        cursor.execute("ALTER TABLE gradeable_component_data DROP CONSTRAINT IF EXISTS gradeable_component_data_verifier_id_fkey")
