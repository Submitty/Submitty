...
class TestConfig(unittest.TestCase):
    ...
    def test_config(self):
        ...
        try:
            ...
            submitty_obj = OrderedDict({"install_dir": "test", "data_dir": "blue"})
            submitty_users_obj = OrderedDict({"dbuser": "submitty_dbuser"})
    ...
    def test_default_driver_config(self):
        temp_dir = tempfile.mkdtemp()
        try:
            database_obj = OrderedDict({"username": "test", "password": "bob", "host": "localhost"})
            submitty_obj = OrderedDict({"install_dir": "test", "data_dir": "blue"})
            submitty_users_obj = OrderedDict({"dbuser": "submitty_dbuser"})
            with Path(temp_dir, "database.json").open("w") as open_file:
                json.dump(database_obj, open_file)
