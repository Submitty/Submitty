...
SAMPLE_ASSIGNMENT_CONFIG = (
    SUBMITTY_INSTALL_DIR + "/more_autograding_examples/python_simple_homework/config"
)
SAMPLE_SUBMISSIONS = (
    SUBMITTY_INSTALL_DIR + "/more_autograding_examples/python_simple_homework/submissions"
)
...
def cleanup(test):
    ...
    subprocess.call(
        ["rm", "-f", os.path.join(test.testcase_path, "data/", "test01/execute_logfile.txt")]
    )
...
def correct(test):
    cleanup(test)
    subprocess.call(["rm"] + ["-f"] + glob.glob(os.path.join(test.testcase_path, "data", "*.py")))
    subprocess.call(
        [
            "cp",
...
def infinite_loop_too_much_output(test):
    ...
    subprocess.call(["rm"] + ["-f"] + glob.glob(os.path.join(test.testcase_path, "data/", "*.py")))
    subprocess.call(
        ["rm"] + ["-f"] + glob.glob(os.path.join(test.testcase_path, "data/", "test01*.txt"))
    )
    subprocess.call(
        [
            "cp",
    ...
    test.diff(
        "test01/execute_logfile.txt", "test01_execute_logfile.txt_infinite_loop_too_much_output"
    )
