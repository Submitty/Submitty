...
def parse_args():
    ...
    parser.add_argument(
        "--users_path",
        default=os.path.join(SETUP_DATA_PATH, "users"),
        help="Path to folder that contains .yml files to use for user creation. "
        "Defaults to ../data/users",
    )
    parser.add_argument(
        "--submission_url", type=str, default="", help="top level url for the website"
    )
    parser.add_argument(
        "--courses_path",
        default=os.path.join(SETUP_DATA_PATH, "courses"),
        help="Path to the folder that contains .yml files to use for course "
        "creation. Defaults to ../data/courses",
    )
    parser.add_argument(
        "--install_dir", type=str, default="/usr/local/submitty", help="install path of submitty"
    )
    parser.add_argument(
        "--data_dir", type=str, default="/var/local/submitty", help="data path of submitty"
    )
    parser.add_argument(
        "course",
        nargs="*",
...
for directory in ["courses"]:
    if not os.path.isdir(os.path.join(SUBMITTY_DATA_DIR, directory)):
        raise SystemError(
            "The following directory does not exist: " + os.path.join(SUBMITTY_DATA_DIR, directory)
        )
