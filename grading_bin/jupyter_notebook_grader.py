...
def execute_notebook(timeout=600):
    ...
    if not notebook_files:
        raise FileNotFoundError(
            "No Jupyter notebook files found in the current directory."
        )
    if len(notebook_files) > 1:
        raise ValueError(
            "Multiple Jupyter notebook files found. Please ensure only one "
            "notebook file is present in the current directory."
        )
