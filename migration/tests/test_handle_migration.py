...
class TestHandleMigration(unittest.TestCase):
    ...
    def add_course(self, database, semester, course, status=1):
        database.execute(f"INSERT INTO courses VALUES ('{semester}', '{course}', {status})")
        database.commit()
    ...
    @patch("migrator.main.migrate_environment")
    @patch("migrator.main.load_triggers")
    def test_migration_multiple_courses_missing_migration(
        self, mock_load_triggers, mock_migrate_environment
    ):
        args = Namespace()
        self.setup_test("course")
    ...
    def test_migration_course_missing_directory(self, mock_load_triggers, mock_migrate_environment):
        ...
        self.assertEqual(
            "Migrator Error:  Could not find directory for f18 csci1100", str(context.exception)
        )
