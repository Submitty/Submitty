...
describe("Docker UI Test", () => {
    ...
    it("Should update the machine information", () => {
        ...
        cy.get(".alert-success")
            ...
            .should(
                "contain",
                "Successfully queued the system to update" +
        ...
        cy.waitAndReloadUntil(() => {
            return cy
                .get('[data-testid="docker_version"]')
                .invoke("text")
                .then((text) => {
                    return text !== "Error";
                });
        ...
        cy.get('[data-testid="systemwide_info"]').should(
            "not.contain.text",
            "Unknown"
        );
        // Updated OS info should not be empty
        cy.get('[data-testid="system_info"]').should("not.be.empty");
        // Updated docker version should not be "Error"
        cy.get('[data-testid="docker_version"]').should(
            "not.contain.text",
            "Error"
        );
    });
    ...
    it("Should filter images with tags", () => {
        ...
        ["cpp", "et-cetera", "notebook", "python"].forEach((tag) => {
            cy.get(`button[data-capability="${tag}"]`).click();
            cy.get(".image-row").should("not.be.visible");
        });
        // Default filter should have all images
        cy.get("button[data-capability='default']").click();
        cy.get(".image-row").should("be.visible");
    });

    it("Should not add invalid image", () => {
        // Check invalid format
        cy.get("#add-field").clear().type("submitty/invalid-image");
        cy.get("#docker-warning").should("be.visible");
        cy.get("#send-button").should("be.disabled");

        // Check valid format but invalid image
        cy.get("#add-field").clear().type("submitty/invalid-image:0.0");
        cy.get("#send-button").should("not.be.disabled").click();

        cy.wait(100);
        cy.get(".alert-error").should(
            "have.text",
            "submitty/invalid-image not found on DockerHub"
        );
    });

    it("Should link existed image to a new tag", () => {
        // Check empty tag list, should have `cpp'
        cy.get("#capabilities-list").contains("cpp");
        // Check valid format and valid image
        cy.get("#capability-form").select("cpp");
        cy.get("#add-field")
            .clear()
        ...
        cy.get("#capabilities-list").should("not.contain.text", "cpp");
    ...
    it("Should add new image", () => {
        ...
        cy.get("#add-field").clear().type("submitty/python:2.7");
        cy.get("#send-button").should("not.be.disabled").click();
        cy.get(".alert-success").should(
            "have.text",
            "submitty/python:2.7 found on DockerHub" +
