    ...
    def add_gradeables(self) -> None:
        ...
        for gradeable in self.gradeables:
            ...
            if gradeable.type == 0 and (
                len(gradeable.submissions) == 0
                or gradeable.sample_path is None
                or gradeable.config_path is None
            ):
            ...
            if gradeable.is_repository:
                ...
                subprocess.check_call(
                    f"sudo {SUBMITTY_INSTALL_DIR}/bin/generate_repos.py {self.semester} {self.code} {gradeable.id}",
                    stdout=subprocess.DEVNULL,
                    stderr=subprocess.STDOUT,
                    shell=True,
                )

            gradeable_annotation_path = os.path.join(
                self.course_path, "annotations", gradeable.id
            )
            ...
            ungraded_section = random.randint(
                1,
                max(
                    1,
                    (
                        self.registration_sections
            ...
            if not NO_SUBMISSIONS:
                only_submit_plagiarized_users = (
                    gradeable.lichen_sample_path is not None
                    and len(gradeable.plagiarized_user) > 0
                )
                for user in self.users:
                    if (
                        only_submit_plagiarized_users
                        and user.id not in gradeable.plagiarized_user
                    ):
                        continue
                    ...
                    if gradeable.team_assignment is True:
                        ...
                        res = self.conn.execute(
                            "SELECT teams.team_id, gradeable_teams.anon_id FROM teams INNER JOIN gradeable_teams"
                        ...
                        if len(temp) != 0:
                            ...
                            previous_submission = select(
                                [self.electronic_gradeable_version]
                            ).where(
                                self.electronic_gradeable_version.c["team_id"]
                                == team_id
                            )
                            ...
                            annotation_path = os.path.join(
                                gradeable_annotation_path, team_id
                            )
                        else:
                            continue
                        res.close()
                        ...
                        annotation_path = os.path.join(
                            gradeable_annotation_path, user.id
                        )
                    ...
                    if gradeable.type == 0 and gradeable.submission_open_date < NOW:
                        if user.id in gradeable.plagiarized_user:
                            ...
                            versions_to_submit = len(
                                gradeable.plagiarized_user[user.id]
                            )
                            ...
                            versions_to_submit = generate_versions_to_submit(
                                max_individual_submissions, max_individual_submissions
                            )

                        if (
                            gradeable.gradeable_config is not None
                            and (
                                gradeable.has_due_date is True
                                and (
                                    gradeable.submission_due_date < NOW
                                    or random.random() < 0.5
                                )
                            )
                            and (random.random() < 0.9)
                            and (
                                max_submissions is None
                                or submission_count < max_submissions
                            )
                        ) or (
                            gradeable.gradeable_config is not None
                            and user.id in gradeable.plagiarized_user
                        ):
                            ...
                            if not os.path.exists(gradeable_path):
                                ...
                                os.system(
                                    f"chown -R submitty_php:{self.code}_tas_www {gradeable_path}"
                                )
                            ...
                            if gradeable.is_repository:
                                if not os.path.exists(checkout_path):
                                    ...
                                    os.system(
                                        f'chown submitty_daemon:{self.code}_tas_www "{checkout_path}"'
                                    )
                                if not os.path.exists(user_checkout_path):
                                    os.makedirs(user_checkout_path)
                                    os.system(
                                        f'chown submitty_daemon:{self.code}_tas_www "{user_checkout_path}"'
                                    )
                            ...
                            version_population = [
                                ver
                                for ver, freq in version_population
                                for i in range(freq)
                            ]
                            ...
                            for version in range(1, versions_to_submit + 1):
                                os.system(
                                    "mkdir -p "
                                    + os.path.join(submission_path, str(version))
                                )
                                submitted = True
                                submission_count += 1
                                current_time_string = dateutils.write_submitty_date(
                                    gradeable.submission_due_date
                                    - timedelta(
                                        days=random_days + version / versions_to_submit
                                    )
                                )
                                if team_id is not None:
                                    self.conn.execute(
                                        self.electronic_gradeable_data.insert(),
                                        g_id=gradeable.id,
                                        user_id=None,
                                        team_id=team_id,
                                        g_version=version,
                                        submission_time=current_time_string,
                                    )
                                    if version == versions_to_submit:
                                        self.conn.execute(
                                            self.electronic_gradeable_version.insert(),
                                            g_id=gradeable.id,
                                            user_id=None,
                                            team_id=team_id,
                                            active_version=active_version,
                                        )
                                    json_history["team_history"] = json_team_history[
                                        team_id
                                    ]
                                else:
                                    self.conn.execute(
                                ...
                                if user.id in gradeable.plagiarized_user:
                                    ...
                                    if (
                                        len(gradeable.plagiarism_submissions) > 0
                                    ):  # check to make sure we haven't run out of data
                                        # if there were no specified plagiarized users but we have plagiarism submissions, grab a random submisison
                                        src = os.path.join(
                                            gradeable.lichen_sample_path,
                                            gradeable.plagiarism_submissions.pop(),
                                        )
                                        dst = os.path.join(
                                            submission_path, str(version)
                                        )
                                        create_gradeable_submission(src, dst)
                                    ...
                                    ).where(self.peer_assign.columns.user_id == user.id)
                                    for res in self.conn.execute(stmt):
                                        assigned_graders.append(res[1])

                                    submissions = random.sample(
                                        gradeable.submissions,
                                        random.randint(1, len(gradeable.submissions)),
                                    )
                                    for submission in submissions:
                                        src = os.path.join(
                                            gradeable.sample_path, submission
                                        )
                                        dst = os.path.join(
                                            submission_path, str(version)
                                        )
                                        ...
                                        if version == versions_to_submit:
                                            annotation_version_path = os.path.join(
                                                annotation_path, str(versions_to_submit)
                                            )
                                            if not os.path.exists(
                                                annotation_version_path
                                            ):
                                                os.makedirs(annotation_version_path)

                                            annotations = random.sample(
                                                gradeable.annotations,
                                                random.randint(
                                                    1, len(gradeable.annotations)
                                                ),
                                            )
                                            graders = (
                                                random.sample(
                                                ...
                                                if len(assigned_graders) > 0
                                                else []
                                            )
                                            # Make sure instructor is responsible for one of the annotations
                                            graders.append("instructor")

                                            anon_dst = os.path.join(
                                                dst, submission
                                            ).split("/")
                                            anon_dst[9] = (
                                                anon_team_id
                                                if team_id is not None
                                                else anon_ids[user.id]
                                            )
                                            anon_dst = "/".join(
                                                anon_dst
                                            )  # has the user id or the team id in the file path being anonymous

                                            for i in range(len(graders)):
                                                annotation_src = os.path.join(
                                                    gradeable.annotation_path,
                                                    annotations[i],
                                                )
                                                annotation_dst = os.path.join(
                                                    annotation_path, str(version)
                                                )
                                                encoded_path = hashlib.md5(
                                                    anon_dst.encode()
                                                ).hexdigest()
                                    ...
                                    if isinstance(gradeable.submissions, dict):
                                        for key in sorted(gradeable.submissions.keys()):
                                            os.system(
                                                "mkdir -p "
                                                + os.path.join(
                                                    submission_path, str(version), key
                                                )
                                            )
                                            submission = random.choice(
                                                gradeable.submissions[key]
                                            )
                                            src = os.path.join(
                                                gradeable.sample_path, submission
                                            )
                                            ...
                                            if gradeable.is_repository:
                                                ...
                                                commit_submission_to_repo(
                                                    ...
                                                    src,
                                                    repo_path,
                                                    gradeable.subdirectory,
                                                )
                                                mimic_checkout(
                                                    repo_path,
                                                    os.path.join(
                                                        user_checkout_path, str(version)
                                                    ),
                                                    gradeable.subdirectory,
                                                )
                                            else:
                            ...
                            if gradeable.is_repository:
                                ...
                                with open(
                                    os.path.join(
                                        submission_path, "user_assignment_settings.json"
                                    ),
                                    "w",
                                ) as open_file:
                                    json.dump(json_history, open_file)

                    if gradeable.grade_start_date < NOW and os.path.exists(
                        os.path.join(submission_path, str(versions_to_submit))
                    ):
                        if (
                            gradeable.has_release_date is True
                            and gradeable.grade_released_date < NOW
                        ) or (
                            random.random() < 0.5 and (submitted or gradeable.type != 0)
                        ):
                            status = 1 if gradeable.type != 0 or submitted else 0
                            print(f"Inserting {gradeable.id} for {user.id}...")
                            values = {"g_id": gradeable.id}
                            overall_comment_values = {
                                "g_id": gradeable.id,
                                "goc_overall_comment": "lorem ipsum lodar",
                                "goc_grader_id": self.instructor.id,
                            }
                            ...
                            if (
                                ...
                                values["gd_user_viewed_date"] = NOW.strftime(
                                    "%Y-%m-%d %H:%M:%S%z"
                                )
                            ...
                            if gradeable.type != 0 or gradeable.use_ta_grading:
                                ...
                                if skip_grading > 0.3 and random.random() > 0.01:
                                    ins = self.gradeable_data_overall_comment.insert().values(
                                        **overall_comment_values
                                    )
                                    res = self.conn.execute(ins)
                                for component in gradeable.components:
                                    ...
                                    if status == 0 or random.random() < 0.4:
                                        ...
                                        uppser_clamp_score = (
                                            random.randint(
                                                component.lower_clamp * 2,
                                                component.upper_clamp * 2,
                                            )
                                            / 2
                                        )
                                        score = generate_probability_space(
                                            {
                                                0.7: max_value_score,
                                                0.2: uppser_clamp_score,
                                                0.08: -max_value_score,
                                                0.02: -99999,
                                            }
                                        )
                                    grade_time = gradeable.grade_start_date.strftime(
                                        "%Y-%m-%d %H:%M:%S%z"
                                    )
                                    self.conn.execute(
                                        self.gradeable_component_data.insert(),
                                        gc_id=component.key,
                                        gd_id=gd_id,
                                        gcd_score=score,
                    ...
                    if gradeable.type == 0 and os.path.isdir(submission_path):
                        os.system(
                            f"chown -R submitty_php:{self.code}_tas_www {submission_path}"
                        )

                    if gradeable.type == 0 and os.path.isdir(gradeable_annotation_path):
                        os.system(
                            f"chown -R submitty_php:{self.code}_tas_www {gradeable_annotation_path}"
                        )

                    if (
                        gradeable.type != 0
                        and gradeable.grade_start_date < NOW
                        and (
                            (
                                gradeable.has_release_date is True
                                and gradeable.grade_released_date < NOW
                        ...
                        and (
                            ungraded_section
                            != (
                                user.get_detail(self.code, "registration_section")
                                if gradeable.grade_by_registration
                                else user.get_detail(self.code, "rotating_section")
                            )
                        )
                    ):
                        res = self.conn.execute(
                            self.gradeable_data.insert(),
                            g_id=gradeable.id,
                            gd_user_id=user.id,
                        )
                        gd_id = res.inserted_primary_key[0]
        ...
        if self.code == "sample":
            ...
            student_image_folder = os.path.join(
                SUBMITTY_DATA_DIR,
                "courses",
                self.semester,
                self.code,
            ...
            zip_path = os.path.join(
                SUBMITTY_REPOSITORY, "sample_files", "user_photos", "CSCI-1300-01.zip"
            )
            with TemporaryDirectory() as tmpdir:
                ...
                for f in os.listdir(inner_folder):
                    shutil.move(
                        os.path.join(inner_folder, f),
                        os.path.join(student_image_folder, f),
                    )
            course_materials_source = os.path.join(
                SUBMITTY_REPOSITORY, "sample_files", "course_materials"
            )
            course_materials_folder = os.path.join(
                SUBMITTY_DATA_DIR,
                "courses",
                self.semester,
                self.code,
            ...
            for dpath, dirs, files in os.walk(course_materials_source):
                ...
                for f in files:
                    ...
                    filepath = os.path.join(
                        ...
                        os.path.relpath(tmpfilepath, course_materials_source),
                    )
