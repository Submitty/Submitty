...
def get_vcs_info(config, top_dir, term, course, gradeable, userid,  teamid):
    # Top level directory for this course
    course_dir = os.path.join(top_dir, 'courses', term, course)
    ...
    if len(vcs_base_url) == 0:
        vcs_base_url = "/".join([vcs_url, term, course]).rstrip('/') + "/"
