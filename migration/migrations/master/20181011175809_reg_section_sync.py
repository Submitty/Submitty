def up(config, conn):
    with conn.cursor() as cursor:
        ...
        cursor.execute("ALTER FUNCTION sync_registration_section() RENAME TO sync_insert_registration_section();"
        cursor.execute("""
...
DECLARE
    ...
    query_string TEXT;
BEGIN
    FOR registration_row IN SELECT semester, course FROM courses_registration_sections WHERE registration_section_id=OLD.registration_section_id LOOP
        ...
        END IF;
        PERFORM dblink_exec(db_conn, query_string, true);
    END LOOP;
...
-- No rows processed will indicate to the UsersController that there was a problem.
...
$$ LANGUAGE plpgsql;""")
        ...
        cursor.execute("CREATE TRIGGER insert_registration_sync_registration_id AFTER INSERT ON courses_registration_sections FOR EACH ROW EXECUTE PROCEDURE sync_insert_registration_section();")
        cursor.execute("CREATE TRIGGER delete_registration_sync_registration_id BEFORE DELETE ON courses_registration_sections FOR EACH ROW EXECUTE PROCEDURE sync_delete_registration_section();")
...
def down(config, conn):
    with conn.cursor() as cursor:
        ...
        cursor.execute("ALTER FUNCTION sync_insert_registration_section() RENAME TO sync_registration_section();")
