...
ORDER BY u.registration_section, u.user_id");
    ...
    // Moved from old system HWReport class, used to grab the gradeables
    // May eventually want to change or eliminate this if we decide to change the HWReport code to use full objects instead of arrays
    
    // NOTE: $arg should be an associative array. To specify the student, use key = 'u_id', and to specify the gradeable, use key = 'g_id'
    public function getReportData($arg = null) {
        $report_query = "SELECT * FROM (
        SELECT
            g_syllabus_bucket,
            g_title,
            g_gradeable_type,
    ...
    WHERE g_gradeable_type='0'";
        ...
        if($arg!=null) {
            if(isset($arg['g_id'])) {
                $where_clause .= " AND g_id=".$arg['g_id'];
            }
            if(isset($arg['u_id'])) {
                $where_clause .= " AND user_id=".$arg['u_id'];
            }
        }
    
        $this->database->query($report_query.$where_clause);
        return $this->database->rows();
    }
    
    // Moved from HWReport class, used to obtain graders info for HWReport Generation
    // May want to remove this function and use getAllGraders() if HWReport is moved to using objects instead of arrays
