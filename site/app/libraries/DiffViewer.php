        ...
        elseif ($actual_file != "") {
            ...
            if (Utils::isImage($actual_file)) {
                ...
                if (filesize($actual_file) < $size_limit) {
                    ...
                    $tmp_actual = file_get_contents($actual_file, false, null, 0, $size_limit);
                    $this->has_actual = trim($tmp_actual) !== "";
                    $this->actual = explode("\n", $tmp_actual);
                    $this->display_actual = true;
                }
        ...
        elseif ($expected_file != "") {
            if (Utils::isImage($expected_file)) {
                ...
                if (filesize($expected_file) < $size_limit) {
                    ...
                    $can_diff = false;
                    //load in the first sizelimit characters of the file (TEMP VALUE)
                    $tmp_expected = file_get_contents($expected_file, false, null, 0, $size_limit);
                    $this->has_expected = trim($tmp_expected) !== "";
                    $this->expected = explode("\n", $tmp_expected);
                    $this->display_expected = true;
                }
    ...
    private function getDisplay($lines, $type = self::EXPECTED, $option = self::SPECIAL_CHARS_ORIGINAL) {
        ...
        if (isset($this->add[$type]) && count($this->add[$type]) > 0) {
            if (array_key_exists(-1, $this->add[$type])) {
                ...
                for ($k = 0; $k < $num_blanks; $k++) {
                    $html .= $type === "expected" ? "\t<div class='row bad-expected'><div class='empty_line'>&nbsp;</div></div>\n" : "\t<div class='row bad'><div class='empty_line'>&nbsp;</div></div>\n";
                }
                $html .= "\t</div>\n";
            }
        }
        ...
        for ($i = 0; $i < count($lines); $i++) {
            ...
            if (isset($this->diff[$type][$i])) {
                $html .= $type === "expected" ? "\t<div class='bad-expected'>" : "\t<div class='bad'>";
            }
            else {
                $html .= "\t<div>";
            }
            ...
            if (isset($this->diff[$type][$i])) {
                ...
                foreach ($this->diff[$type][$i] as $diff) {
                    ...
                    if ($option == self::SPECIAL_CHARS_ORIGINAL) {
                        $html .= $html_orig;
                        $html .= $type === "expected" ? "<span class='highlight-char-expected'>" . $html_orig_error . "</span>" :  "<span class='highlight-char'>" . $html_orig_error . "</span>";
                    }
                    elseif ($option == self::SPECIAL_CHARS_UNICODE) {
                        ...
                        $html .= $html_no_empty;
                        $html .= $type === "expected" ? "<span class='highlight-char-expected'>" . $html_no_empty_error . "</span>" : "<span class='highlight-char'>" . $html_no_empty_error . "</span>";
                    }
            ...
            if (isset($this->add[$type][$i])) {
                ...
                for ($k = 0; $k < $this->add[$type][$i]; $k++) {
                    $html .= $type === "expected" ? "\t<div class='bad-expected'><td class='empty_line' colspan='2'>&nbsp;</td></div>\n" : "\t<div class='bad'><td class='empty_line' colspan='2'>&nbsp;</td></div>\n";
                }
                if ($start === null) {
                    $html .= "\t</div>\n";
                }
