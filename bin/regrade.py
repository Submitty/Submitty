...
def replay(starttime, endtime):
    ...
    with open(file, 'r') as lines:
        for line in lines:
            ...
            if (original_time < starttime or original_time > endtime):
                continue
            # skip batch items
...
def main():
    ...
    for input_path in args.path:
        ...
        pattern = os.path.join(pattern_semester, pattern_course, "submissions",
                               pattern_gradeable, pattern_who, pattern_version)
        ...
        for d in Path(data_dir).glob(pattern):
            ...
            if os.path.isdir(d):
                ...
                gradeable_config = os.path.join(data_dir, my_semester, my_course,
                                                "config/build/" + "build_" + my_gradeable + ".json")
                ...
                my_path = os.path.join(data_dir, my_semester, my_course, "submissions",
                                       my_gradeable, my_who, my_version)
                if my_path != d:
