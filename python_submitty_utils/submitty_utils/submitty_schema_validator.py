...
class SubmittySchemaException(Exception):
    ...
    def print_human_readable_error(self):
        ...
        if self.schema_message is not None:
            ...
            print(("The portion of your configuration that caused the error is:"), file=sys.stderr)
...
def complete_config_validator(j_, s_, warn=True):
    ...
    for key in ["autograding", "autograding_method", "container_options", "resource_limits"]:
...
def validate_schema(j_, s_, key="", prefix="", required=True, warn=False):
    ...
    if my_schema is None:
        msg = (
            f"ERROR: There is no specification for {key} in the schema.",
            " Please add a specification.",
        )
        raise SubmittySchemaException(j_, s_, msg, descriptive_title, None)
    ...
    try:
        ...
        raise SubmittySchemaException(my_json_chunk, my_schema, msg, descriptive_title, e)


def validate_complete_config_schema_using_filenames(config_path, schema_path, warn=False):
    """
