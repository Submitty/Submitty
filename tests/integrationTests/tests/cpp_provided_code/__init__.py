...
SAMPLE_ASSIGNMENT_CONFIG = (
    SUBMITTY_INSTALL_DIR + "/more_autograding_examples/cpp_provided_code/config"
)
SAMPLE_SUBMISSIONS = (
    SUBMITTY_INSTALL_DIR + "/more_autograding_examples/cpp_provided_code/submissions"
)
...
def initialize(test):
    ...
    shutil.copytree(SAMPLE_ASSIGNMENT_CONFIG, os.path.join(test.testcase_path, "assignment_config"))
...
def solution(test):
    cleanup(test)
    shutil.copy(
        os.path.join(SAMPLE_SUBMISSIONS, "student.h"), os.path.join(test.testcase_path, "data")
    )
    shutil.copy(
        os.path.join(SAMPLE_SUBMISSIONS, "student.cpp"), os.path.join(test.testcase_path, "data")
    )
    ...
    test.run_run()
    shutil.copy(
        os.path.join(test.testcase_path, "assignment_config", "test_output", "output.txt"),
