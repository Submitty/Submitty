...
if course is None:
    raise SystemExit("term '{}' and Course '{}' not found".format(args.term, args.course))

vcs_term = os.path.join(VCS_FOLDER, args.term)
if not os.path.isdir(vcs_term):
    os.makedirs(vcs_term, mode=0o770, exist_ok=True)
    shutil.chown(vcs_term, group=DAEMONCGI_GROUP)

vcs_course = os.path.join(vcs_term, args.course)
...
if eg is not None:
    ...
    print ("Warning: term '{}' and Course '{}' does not contain gradeable_id '{}'.".format(args.term, args.course, args.repo_name))
