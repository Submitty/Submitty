...
if [[ $(cut -d ':' -f 1 /etc/group | grep -c ${DAEMONPHP_GROUP}) -eq "0" ]]; then
...
# management scripts.
if [[ $(cut -d ':' -f 1 /etc/group | grep -c ${COURSE_BUILDERS_GROUP}) -eq "0" ]]; then
...
if [[ $(cut -d ':' -f 1 /etc/passwd | grep -c ${DAEMON_USER}) -eq "0" ]];then
