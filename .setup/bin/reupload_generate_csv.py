...
def main():
    parser = argparse.ArgumentParser(
        description="Given a directory mirroring a submission directory (see arguments),\
     this script will generate a csv userlist for upload to submitty."
    )
    parser.add_argument(
        "archived_directory",
        help="The path to the top level of the old assignment's directory tree.\
    ...
    if not os.path.isdir(args.archived_directory):
        raise SystemExit(
            "The provided directory (" + args.archived_directory + ") does not exist."
        )
