...
def main():
    ...
    select_query = select(courses_table).where(
        and_(
            courses_table.c.term == bindparam("term"), courses_table.c.course == bindparam("course")
        )
    )
    ...
    select_query = select(courses_u_table).where(
        and_(
            and_(
    ...
    row = connection.execute(
        select_query, {"term": semester, "course": course, "user_id": user_id}
    ).fetchone()
