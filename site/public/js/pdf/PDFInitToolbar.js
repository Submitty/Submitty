...
function renderPDFToolbar() {
    ...
    function saveFile() {
        const GENERAL_NFORMATION = window.GENERAL_INFORMATION;
        const url = buildCourseUrl(['gradeable', GENERAL_NFORMATION['gradeable_id'], 'pdf', 'annotations']);
        const annotation_layer = localStorage.getItem(`${window.RENDER_OPTIONS.documentId}/${GENERAL_INFORMATION.grader_id}/annotations`) || {};
        $.ajax({
            type: 'POST',
            url: url,
            data: {
    ...
    function colorMenuToggle(e) {
        const shouldShow = !$('#color_selector_menu').is(':visible');
        $('.selection-menu').hide();
    ...
    function sizeMenuToggle(e) {
        const shouldShow = !$('#size_selector_menu').is(':visible');
        $('.selection-menu').hide();
    ...
    function initPen() {
        ...
        document.getElementById('pen_size_selector').addEventListener('change', (e) => {
    ...
    function initText() {
        const init_size = localStorage.getItem('text/size') || 12;
        const init_color = localStorage.getItem('main_color') || '#000000';
        ...
        document.getElementById('text_size_selector').addEventListener('change', (e) => {
