...
class NumericGraderController extends AbstractController  {
    public function run() {
    ...
    public function dataEntry() {
        ...
        if(count($sections) === 0 && (!isset($_GET['view']) || $_GET['view'] !== "all") && !$this->core->getUser()->accessAdmin()){
            $this->core->getOutput()->renderOutput(array('grading', 'NumericGrader'), 'numericText', $gradeable, $sections, $graders);
            return;
        }
        ...
        $this->core->getOutput()->renderOutput(array('grading', 'NumericGrader'), 'numericText', $gradeable, $rows, $graders);
    }
