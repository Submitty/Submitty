...
SAMPLE_ASSIGNMENT_CONFIG = os.path.join(
    SUBMITTY_INSTALL_DIR, "more_autograding_examples", "c_system_call_filtering", "config"
)
SAMPLE_SUBMISSIONS = os.path.join(
    SUBMITTY_INSTALL_DIR, "more_autograding_examples", "c_system_call_filtering", "submissions"
)
...
def restricted_device_management(test):
    ...
    subprocess.call(
        ["rm", os.path.join(test.testcase_path, "data", "restricted_device_management.c")]
    )
