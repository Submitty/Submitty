...
class LateDayView extends AbstractView {
    ...
    public function displayLateDayCache($students, $initial_late_days) {
        ...
        $this->core->getOutput()->addVendorJs(FileUtils::joinPaths('flatpickr', 'plugins', 'shortcutButtons', 'shortcut-buttons-flatpickr.min.js'));
        $this->core->getOutput()->addVendorCss(FileUtils::joinPaths('flatpickr', 'plugins', 'shortcutButtons', 'themes', 'light.min.css'));
        $this->core->getOutput()->addBreadcrumb('Late Day Cache');
        $this->core->getOutput()->enableMobileViewport();
        ...
        return $this->core->getOutput()->renderTwigTemplate("admin/late_day_cache/LateDayCacheBase.twig", [
            ...
            "students" => $students,
            "initial_late_days" => $initial_late_days,
            "flush_cache_url" => $this->core->buildCourseUrl(['late_day_cache', 'flush']),
            "calculate_cache_url" => $this->core->buildCourseUrl(['late_day_cache', 'calculate']),
            "cache_url" => $this->core->buildCourseUrl(['late_day_cache']),
            "view_all" => $view_all,
