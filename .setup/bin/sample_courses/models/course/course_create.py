...
class Course_create:
    ...
    def create(self) -> None:
        ...
        for section in range(1, self.registration_sections+1):
            ...
            submitty_conn.execute(table.insert(), term=self.semester, course=self.code,
        ...
        for user in self.users:
            ...
            if user.get_detail(self.code, "grading_registration_section") is not None:
                try:
                    grading_registration_sections = str(user.get_detail(
                        self.code, "grading_registration_section"))
                ...
                for grading_registration_section in grading_registration_sections:
                    self.conn.execute(reg_table.insert(),
                                      user_id=user.get_detail(self.code, "id"),
                                      sections_registration_id=str(
                                          grading_registration_section))
        ...
        self.electronic_table = Table("electronic_gradeable", self.metadata, autoload=True)
        ...
        self.component_table = Table('gradeable_component', self.metadata, autoload=True)
        self.mark_table = Table('gradeable_component_mark', self.metadata, autoload=True)
        self.gradeable_data = Table("gradeable_data", self.metadata, autoload=True)
        self.gradeable_component_data = Table("gradeable_component_data", self.metadata,
                                              autoload=True)
        self.gradeable_component_mark_data = Table('gradeable_component_mark_data', self.metadata,
                                                   autoload=True)
        self.gradeable_data_overall_comment = Table('gradeable_data_overall_comment',
                                                    self.metadata, autoload=True)
        self.electronic_gradeable_data = Table("electronic_gradeable_data", self.metadata,
                                               autoload=True)
        self.electronic_gradeable_version = Table("electronic_gradeable_version", self.metadata,
                                                  autoload=True)
        for gradeable in self.gradeables:
            gradeable.create(self.conn, self.gradeable_table, self.electronic_table,
                             self.peer_assign, reg_table, self.component_table, self.mark_table)
        ...
        os.system(f"chown -f submitty_php:{self.code}_tas_www "
        ...
        if not os.path.isfile(os.path.join(self.course_path, "ASSIGNMENTS.txt")):
            ...
            os.system(f"chown {self.instructor.id}:{self.code}_tas_www "
                      f"{os.path.join(self.course_path, 'ASSIGNMENTS.txt')}")
            os.system(f"chmod -R g+w {self.course_path}")
            os.system(f"su {'submitty_daemon'} -c "
                      f"'{ os.path.join(self.course_path,f'BUILD_{self.code}.sh')}'")
        os.system(f"chown -R {self.instructor.id}:{self.code}_tas_www "
