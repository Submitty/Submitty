...
def do_the_grading():
    ...
    with open(my_submission_file) as submission_file:
        ...
        submission_timestamp_string = contents.strip()
    ...
    if hours > 0:
        ...
        duration_string = f"{minutes} minutes {seconds} seconds"
    else:
        duration_string = f"{seconds} seconds"

    # Adjust allowed minutes if remaining time is less than allowed
    current_time = datetime.now(tz=tzlocal.get_localzone())
    due_date = submission_timestamp - timedelta(seconds=access_duration)
    remaining_time = int((due_date - current_time).total_seconds() / 60)
    allowed_minutes = min(allowed_minutes, remaining_time)

    # Compute the number of minutes (round up) in excess of the allowed time
    penalty_minutes = hours * 60 + minutes
    if seconds > 0:
        penalty_minutes += 1
    penalty_minutes -= allowed_minutes
    ...
    my_message = f"version: {active_version}" \
                 f"\nfirst access: {first_access_timestamp}" \
                 f"\nsubmission: {submission_timestamp}" \
                 f"\nallowed minutes: {allowed_minutes}" \
                 f"\nduration: {duration_string}" \
                 f"\npenalty minutes: {penalty_minutes}" \
                 f"\npenalty per minute: {penalty_per_minute}" \
                 f"\ndeduction (before rounding): {my_points_deducted}" \
                 f"\ndeduction (after rounding): {my_score * max_penalty}"
