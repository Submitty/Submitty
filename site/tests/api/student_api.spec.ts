...
describe('Tests cases for the Student API', () => {
    it('Should get correct responses', () => {
        ...
        Promise.resolve(postRequest('/api/token', {
            ...
            getRequest(`/api/${getCurrentSemester(false)}/sample/gradeable/subdirectory_vcs_homework/values?user_id=not_a_student`,
            ).then((response) => {
        ...
        Promise.resolve(postRequest('/api/token', {
            ...
            // Success
            getRequest(`/api/${getCurrentSemester(false)}/sample/gradeable/subdirectory_vcs_homework/values?user_id=student`,
                ).then((response) => {
                expect(response.body.status).to.equal('success');
                // Can't test exact values due to randomness of CI speed
            ...
            postRequest(`/api/${getCurrentSemester(false)}/sample/gradeable/subdirectory_vcs_homework/grade`,
                {
                    'user_id': 'student',
                    'vcs_checkout': 'true',
                    'git_repo_id': 'none',
