...
TestResults* DrMemoryGrader_doit (const TestCase &tc, const nlohmann::json& j) {
  ...
  if (!drmemory_output.good()) {
    return new TestResults(0.0,{"ERROR: DrMemory output does not exist"});
  }
  ...
  if (num_errors > 0) {
    messages.push_back(std::to_string(num_errors) + " DrMemory Errors");
    result = 0;
  }
  if (result > 0.01 && 
    ...
    messages.push_back("Program Contains Memory Errors");
    result = 0;
  }
  if (no_errors_message == true &&
      result < 0.99) {
    messages.push_back("Your Program *does* contains memory errors (misleading DrMemory Output \"NO ERRORS FOUND\")");
  }
