...
if __name__ == "__main__":
    ...
    try:
        ...
        current_short_commit_hash = subprocess.check_output(
            ["git", "rev-parse", "--short", "HEAD"], cwd=repository_dir
        )
        current_short_commit_hash = current_short_commit_hash.decode("ascii").strip()
        print(
            "Commit {0} is currently installed on this system.".format(
                current_commit_hash
            )
        )
    except Exception:
    ...
    try:
        ...
        current_git_tag = subprocess.check_output(
            ["git", "describe", "--tag", "--abbrev=0"], cwd=repository_dir
        )
