...
const emit = defineEmits<{
    /** Fired whenever the date => filename map changes, so buildJSON() can pick it up */
...
async function openRoster(date: string) {
    ...
    // Always re-read on open rather than trusting what the page rendered -- another
    // tab or the nightly build may have changed the file underneath us.
    try {
...
async function saveRoster() {
    ...
    try {
        roster.value = uploadFile.value !== null
            ...
            : await saveBonusLateDayRoster(date, roster.value);
...
<template>
  ...
  >
    ...
    <p class="rg_info_message">
      Award one additional late day to a subset of students, available starting on the
      selected date.  A bonus dated in the future will not appear in the gradebook until
      that date arrives.
    </p>
      ...
      >
        ...
        >
          ...
          >
            ...
            <td>
              <a
                :data-testid="`bonus-late-days-delete-${date}`"
