...
class IniParser {
    ...
    public static function readFile($filename) {
        if (!function_exists('parse_ini_file')) {
            throw new \RuntimeException("parse_ini_file needs to be enabled");
        }

        if (!file_exists($filename)) {
            throw new \RuntimeException("Could not find ini file to parse: {$filename}");
        }
        ...
        if ($parsed === false) {
            ...
            throw new \RuntimeException("Error reading ini file '{$basename}': {$e['message']}");
        }
    ...
    public static function writeFile($filename, $config) {
                    ...
                    foreach ($value as $kkey => $vvalue) {
                        if (is_array($vvalue)) {
                            foreach ($vvalue as $vvvalue) {
                                if (is_array($vvvalue)) {
                                    throw new \RuntimeException("Cannot have nested arrays in ini files");
                                }
        ...
        if (@file_put_contents($filename, $to_write) === false) {
            throw new \RuntimeException("Could not write ini file {$filename}");
        }
    }
