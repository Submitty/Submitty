 ...
 * @return {Object}
 */
async function ajaxAddComponent(gradeable_id: string | undefined, peer: boolean, curve: boolean) {
    let response: Record<string, string | undefined> | null;
    try {
        response = await $.ajax({
            dataType: 'json',
 ...
 */
window.onAddComponent = async function (peer: boolean, curve: boolean) {
    try {
        await addComponent(peer, curve);
    }
    catch (err) {
        console.error(err);
 ...
 * @return {Promise}
 */
async function addComponent(peer: boolean, curve: boolean) {
