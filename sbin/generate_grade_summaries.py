...
def save_and_build_rainbow_grades(semester, course, token):
    ...
    if save_response.status_code == 200 and save_response.json()["status"] == "success":
        print(
            "Successfully saved Rainbow Grades GUI customization for {}.{}".format(semester, course)
        )
    else:
        ...
        if message == "Manual customization is currently in use.":
            ...
            print(
                "ERROR: Failed to save Rainbow Grades GUI customization for {}.{} - {}".format(
    ...
    select_response = requests.post(
        ...
        headers={"Authorization": token},
        data={"selected_value": option, "source": "submitty_daemon"},
    )

    if select_response.status_code == 200 and select_response.json()["status"] == "success":
        print(
            "Successfully selected the {} customization for {}.{}".format(option, semester, course)
        )
    else:
        print(
            "ERROR: Failed to select the {} customization for {}.{} - {}".format(
    ...
    build_response = requests.post(
        "{}/api/courses/{}/{}/reports/build_form".format(base_url, semester, course),
        headers={"Authorization": token, "source": "submitty_daemon"},
    )

    if build_response.status_code == 200 and build_response.json()["status"] == "success":
        print(
            "Successfully submitted the Rainbow Grades build process for {}.{}".format(
                semester, course
            )
        )
    else:
        print(
            "ERROR: Failed to submit the Rainbow Grades build process for {}.{} - {}".format(
    ...
    status_response = requests.post(
        "{}/api/courses/{}/{}/reports/rainbow_grades_status".format(base_url, semester, course),
        headers={"Authorization": token},
    )
    print(
        "Successfully completed the Rainbow Grades build process for {}.{} - {}".format(
...
def generate_grade_summaries(semester, course, token):
    ...
    try:
        grade_generation_response = requests.post(
            "{}/api/courses/{}/{}/reports/summaries".format(base_url, semester, course),
            headers={"Authorization": token},
        )
        ...
        if grade_generation_response.status_code == 200:
            ...
            if grade_generation_response["status"] == "success":
                print("Successfully generated grade summaries for {}.{}".format(semester, course))
            else:
                print(
                    "ERROR: Failed to generate grade summaries for {}.{} - {}".format(
