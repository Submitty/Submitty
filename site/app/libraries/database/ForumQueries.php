...
class ForumQueries {
 //extends DatabaseQueries {
    ...
    public function createPost(Post &$p, bool $isFirst) {
        ...
        try {
            $this->course_db->query("INSERT INTO posts (thread_id, parent_id, author_user_id, content, timestamp, anonymous, deleted, endorsed_by, type, has_attachment) VALUES (?, ?, ?, ?, current_timestamp, ?, ?, ?, ?, ?)", [$p->getThreadId(), $p->getParentId(), $p->getAuthor(), $p->getContent(), $anonymous, 0, null, $type, $hasAttachment]);
    ...
    public function createThread(Thread &$t) {
        ...
        try {
            $this->course_db->query("INSERT INTO threads (title, created_by, pinned, status, deleted, merged_thread_id, merged_post_id, is_visible) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", [$t->getTitle(), $t->getUser(), $t->isPinned(), $t->getStatus(), 0, -1, -1, true]);
        }
        catch (DatabaseException $dbException) {
            $this->course_db->rollback();
            return false;
        ...
        $this->course_db->query("SELECT MAX(id) as max_id from threads where title=? and created_by=?", [$t->getTitle(), $t->getUser()]);
