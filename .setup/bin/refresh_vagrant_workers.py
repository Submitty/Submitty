...
if "version" in vagrant_workers_data and type(vagrant_workers_data["version"]) is int:
    ...
    print(
        "This script requires a worker configuration of v2 or greater. Please regenerate your configuration with 'vagrant workers generate'."
    )
    exit(1)
...
DAEMON_HOME = os.path.realpath(
    subprocess.check_output(["su", DAEMON_USER, "-c", "echo $HOME"]).strip()
)
...
subprocess.run(
    ["su", DAEMON_USER, "-c", "ssh-keygen -b 2048 -t rsa -f ~/.ssh/id_rsa -q -N ''"], check=True
)
...
for name, data in vagrant_workers_data.items():
    ...
    shutil.copyfile(
        f"{SUBMITTY_REPOSITORY}/.vagrant/machines/{name}/{provider}/private_key",
        f"/tmp/worker_keys/{name}",
    )
    ...
    w = subprocess.run(
        [
            ...
            DAEMON_USER,
            "-c",
            f"scp -i /tmp/worker_keys/{name} -o StrictHostKeyChecking=no ~/.ssh/id_rsa.pub root@{data['ip_addr']}:/tmp/workerkey",
        ]
    )
    if w.returncode != 0:
    ...
    w = subprocess.run(
        [
            ...
            DAEMON_USER,
            "-c",
            f'ssh -i /tmp/worker_keys/{name} -o StrictHostKeyChecking=no root@{data["ip_addr"]} "chown {SUPERVISOR_USER}:{SUPERVISOR_USER} /tmp/workerkey && su {SUPERVISOR_USER} -c \\"mkdir -p ~/.ssh && mv /tmp/workerkey ~/.ssh/authorized_keys\\""',
        ]
    )
    if w.returncode == 0:
