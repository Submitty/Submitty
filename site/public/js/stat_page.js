...
$('button').click(function () {
    ...
    let thread_ids = $(this).data('thread_id');
    let thread_titles = $(this).data('thread_titles');
    if (action == 'expand') {
        ...
        if ($(this).data('type') == 'post' || $(this).data('type') == 'thread') {
            ...
            let url = buildCourseUrl(['forum', 'threads', id]);
            window.open(url);
        }
        ...
        for (let j = 0; j < ids.length; j++) {
            let item = document.getElementById(ids[j]);
            item.remove();
        }
