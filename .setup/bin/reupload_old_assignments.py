...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="This script was created to help professors re-upload an old semester's\
    ...
    parser.add_argument(
        ...
        "--grade",
        action="store_true",
        help="adds assignments to the grading queue.",
    )
    parser.add_argument(
        "ARCHIVED_directory",
        help="The path to the top level of the old assignment's directory tree. This folder's\
        subdirectories should mirror a /var/local/submitty/courses/<semester>/<course>/submissions/<assignment_name> folder.",
    )
    parser.add_argument(
        "semester", help="The semester of the course you wish to upload to."
    )
    parser.add_argument(
        "course_name", help="The name of the course you wish to upload to."
    )
    parser.add_argument(
        "assignment_name", help="The assignment name you wish to upload to."
    )
    args = parser.parse_args()

    print(
        "You are about to attempt to copy\n\tSOURCE: "
    ...
    CURRENT_course_path = os.path.join(
        SUBMITTY_DATA_DIR, "courses", args.semester, args.course_name
    )
    print("Current course path:" + CURRENT_course_path)
    CURRENT_assignment_path = os.path.join(
        CURRENT_course_path, "submissions", args.assignment_name
    )
    ...
    CURRENT_bin_directory = os.path.join(
        CURRENT_course_path, "bin", args.assignment_name
    )
    if not os.path.isdir(CURRENT_bin_directory):
        raise SystemExit(
            "ERROR: The directory "
            + CURRENT_bin_directory
            + " does not exist. Please make sure that you\n\t\
    ...
    engine = create_engine(
        "postgresql://{}:{}@{}/{}".format(DB_USER, DB_PASS, DB_HOST, database)
    )
    conn = engine.connect()
    metadata = MetaData(bind=engine)
    print("(connection made, metadata bound)...")
    electronic_gradeable_data = Table(
        "electronic_gradeable_data", metadata, autoload=True
    )
    electronic_gradeable_version = Table(
        "electronic_gradeable_version", metadata, autoload=True
    )

    course_group = grp.getgrgid(
        os.stat(
            os.path.join(SUBMITTY_DATA_DIR, "courses", args.semester, args.course_name)
        ).st_gid
    )[0]
    ...
    for user_folder in sorted(os.listdir(args.ARCHIVED_directory)):
        ...
        if not os.path.isdir(ARCHIVED_user_dir):
            print(
                "Skipping the following as it is not a directory: " + ARCHIVED_user_dir
            )
            continue
        ...
        with open(
            os.path.join(ARCHIVED_user_dir, "user_assignment_settings.json"), "r"
        ) as open_file:
        ...
        for submission in os.listdir(ARCHIVED_user_dir):
            ...
            if not os.path.isdir(ARCHIVED_submission_path):
                if user_folder == ".submit.timestamp":
                    with open(ARCHIVED_submission_path, "r") as old_timestamp, open(
                        CURRENT_user_path, "w"
                    ) as new_timestamp:
                        for line in old_timestamp:
                            new_timestamp.write(line)
                        os.system(
                            "chown -R submitty_php:{} {}".format(
                                course_group, new_timestamp
                            )
                        )
                continue
            ...
            os.system(
                "chown -R submitty_php:{} {}".format(course_group, CURRENT_user_path)
            )
            ...
            os.system(
                "chown -R submitty_php:{} {}".format(
                    course_group, CURRENT_submission_path
                )
            )
            ...
            conn.execute(
                electronic_gradeable_data.insert(),
                g_id=args.assignment_name,
                user_id=user_name,
                g_version=submission,
            ...
            if int(submission) == 1:
                print(
                    "Entered new user "
                    + user_name
                    + " because submission was "
                    + submission
                )
                conn.execute(
                    electronic_gradeable_version.insert(),
                    g_id=args.assignment_name,
                    user_id=user_name,
                ...
                print(
                    "UPDATED: where g_id is "
                    + args.assignment_name
                    + " and user id is "
                    + user_name
                    + " to value "
                    + str(user_assignment_settings["active_version"])
                )
                stmt = (
                    electronic_gradeable_version.update()
                    .where(
                        and_(
                            electronic_gradeable_version.c.g_id == args.assignment_name,
                            electronic_gradeable_version.c.user_id == user_name,
                        )
                    )
                ...
                conn.execute(stmt)
            with open(
                os.path.join(CURRENT_user_path, "user_assignment_settings.json"), "w"
            ) as open_file:
            ...
            if args.grade:
                ...
                queue_file = os.path.join(
                    SUBMITTY_DATA_DIR, "to_be_graded_queue", queue_file
                )
