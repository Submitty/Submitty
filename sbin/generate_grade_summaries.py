...
def main():
    ...
    courses = list()
    course_response = requests.get('{}/api/courses'.format(base_url), headers={'Authorization': token})
    ...
    for course in courses:
        grade_generation_response = requests.post('{}/api/{}/{}/reports/summaries'.format(base_url, course['semester'], course['title']), headers={'Authorization': token})
        if grade_generation_response.status_code == 200:
            ...
            if grade_generation_response["status"] == 'success':
                print("Successfully generated rainbow grades for {}.{}".format(course['semester'], course['title']))
            else:
                print("ERROR: Failed to generate rainbow grades for {}.{}.\nReason:{}".format(course['semester'], course['title'], grade_generation_response["message"]))
        else:
            print("ERROR: Submitty Service Unavailable.")
