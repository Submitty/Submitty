...
function build_homework {
    ...
    # where it should be installed (what term, course, and assignment number/name)
    term="$2"
    course="$3"
    assignment="$4"
    ...
    python3 "${SUBMITTY_INSTALL_DIR}/bin/set_allowed_mins.py" "${hw_build_path}/complete_config.json" "${term}" "${course}" "${assignment}"
    set_minutes="$?"
    ...
    "$SUBMITTY_INSTALL_DIR"/bin/config_syntax_check.py "$hw_source" "$assignment" "$term" "$course"

    # generate queue file for generated_output
    "$SUBMITTY_INSTALL_DIR"/bin/make_generated_output.py "$hw_source" "$assignment" "$term" "$course"
