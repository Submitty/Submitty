...
def sample_rainbow_grades_test():
    ...
    for f in os.listdir(runner_dir):
        if "__sample__" in f:
            error_and_cleanup(
                test_tmp,
                "sample has assignments in the grading queue."
    ...
    try:
        shutil.copy(
            os.path.join(script_path, "Makefile_sample"), os.path.join(summary_tmp, "Makefile")
        )
        shutil.copy(
            os.path.join(
                "__INSTALL__FILLIN__SUBMITTY_INSTALL_DIR__", ".setup", "customization_sample.json"
            ),
    ...
    return_code = subprocess.call(
        [
            "tar",
            "-xf",
            os.path.join(script_path, "raw_data_10090542_sample.tar"),
            "-C",
            known_raw_path,
        ]
    )
    ...
    if len(known_files) != len(summary_files):
        ...
        if len(summary_files) == 0:
            ...
            error_and_cleanup(
                test_tmp,
                "There are {} more files in the rsync'd raw_data than expected.".format(
                    -1 * file_diff
                ),
            )
    ...
    try:
        with (
            open(os.path.join(summary_tmp, "output.html"), "r") as output_generated_file,
            open(
                os.path.join(script_path, "output_10090542_sample.html"), "r"
            ) as output_known_file,
        ):
    ...
    if len(known_files) != len(summary_files):
        ...
        if len(known_files) > len(summary_files):
            ...
            error_and_cleanup(
                test_tmp,
                "There are {} fewer files in the generated ".format(-1 * file_diff)
                + "individual_summary_html than expected.",
            )
