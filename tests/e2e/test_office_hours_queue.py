...
class TestOfficeHoursQueue(BaseTestCase):
    ...
    def __init__(self, testname):
        super().__init__(
            ...
            user_name="Quinn",
            use_websockets=True,
            socket_page="office_hours_queue",
        )
    ...
    def test_office_hours_queue(self):
        ...
        self.expectedAlerts(
            ...
            1,
            success_text=[],
            error_text=["Unable to add queue. Make sure you have a unique queue name"],
        )
        ...
        self.expectedAlerts(
            ...
            success_text=[],
            error_text=[
                "Cannot restore a user that is currently in the queue. Please remove them first."
            ],
        )
        self.assertEqual(base_queue_history_count + 2, self.queueHistoryCount(False))
        self.assertEqual(2, self.currentQueueCount())
        ...
        self.assertEqual(
            " ".join(self.driver.find_element(By.ID, "announcement").text.split()),
            f"Office Hours Queue Announcements: {announcement_string}",
        )
        ...
        self.assertEqual(
            ...
            self.verifyElementMissing(
                "id",
                [
                    "toggle_filter_settings",
                    "new_queue_code",
                    "new-queue-token",
                    "new_queue_rand_token",
                    "open_new_queue_btn",
                ],
            ),
        )
    ...
    def openFilterSettings(self):
        self.goToQueuePage()
        self.assertEqual(
            True, self.driver.execute_script("return $('#filter-settings').is(':hidden')")
        )
        self.driver.find_element(By.ID, "toggle_filter_settings").click()
        self.assertEqual(
            False, self.driver.execute_script("return $('#filter-settings').is(':hidden')")
        )
    ...
    def closeFilterSettings(self):
        ...
        self.driver.find_element(
            By.XPATH, '//*[@id="filter-settings"]//*[@class="form-button-container"]/*'
        ).click()
        self.assertEqual(
            True, self.driver.execute_script("return $('#filter-settings').is(':hidden')")
        )
    ...
    def openAnnouncementSettings(self):
        self.goToQueuePage()
        self.assertEqual(
            True, self.driver.execute_script("return $('#announcement-settings').is(':hidden')")
        )
        self.driver.find_element(By.ID, "toggle_announcement_settings").click()
        self.assertEqual(
            False, self.driver.execute_script("return $('#announcement-settings').is(':hidden')")
        )
    ...
    def changeQueueCode(self, name, code=None):
        self.openFilterSettings()
        self.driver.find_element(
            By.XPATH, f'//*[@id="old_queue_code"]/option[text()="{name}"]'
        ).click()
        if code:
    ...
    def studentJoinQueue(self, queueName, queueCode, studentName=None):
        ...
        self.assertIn(
            urllib.parse.quote(queueName),
            self.driver.find_element(By.ID, "add_to_queue").get_attribute("action"),
        )
        self.assertEqual(
            queueCode, self.driver.find_element(By.ID, "token-box").get_attribute("value")
        )
