...
TestResults* PacmanGrader_doit (const TestCase &tc, const nlohmann::json& j) {
  ...
  if (!pacman_output.good()) {
    return new TestResults(0.0,{"ERROR: Pacman output does not exist"});
  }

  // instructor must provided correct expected number of tests
  int num_pacman_tests = j.value("num_tests",-1);
  if (num_pacman_tests <= 0) {
    return new TestResults(0.0,{"CONFIGURATION ERROR: Must specify number of Pacman tests"});
  }
  ...
  if (total_possible != check_possible ||
    ...
    messages.push_back("ERROR: Summation of parsed points does not match");
    return new TestResults(0.0,messages);
  }

  // final answer
  messages.push_back("Total: " + std::to_string(total_awarded) + " / " + std::to_string(total_possible));
