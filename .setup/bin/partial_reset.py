...
def main():
    ...
    if cmd_exists('psql'):
        ...
        os.system("psql localhost -U hsdbu --list | grep submitty* | awk '{print $1}' | "
                  "xargs -I \"@@\" dropdb -h localhost -U hsdbu \"@@\"")
        os.system('psql localhost -d postgres -U hsdbu -c "CREATE DATABASE submitty"')
        os.system('psql localhost -d submitty -U hsdbu -f {}/site/data/submitty_db.sql'.format(SUBMITTY_REPOSITORY))
