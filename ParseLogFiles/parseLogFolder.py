...
avg_gradingtime_ot_week = [[0 for x in range(24)] for x in range(7)]##avergage grade time per assignment per week
...
for filename in os.listdir(path):
	...
	for line in contents.readlines():
		...
		if(line[40:45] != "BATCH"):
			...
			if(new_day == True):
				...
				number_of_weekdays[day_of_week]= number_of_weekdays[day_of_week]+1
			hour = int(line[11:13])
			...
			if(line[101:105] == "wait"):
				...
				avg_waittime_ot_week[day_of_week][hour] = avg_waittime_ot_week[day_of_week][hour] + int(line[106:114])

			elif(line [101:106]=="grade"):
				avg_gradingtime_ot_week[day_of_week][hour] = avg_gradingtime_ot_week[day_of_week][hour] + int(line[107:114])

			if(info[1]=="csci1200"):#data Structures
				submission_temp_cs1200 = submission_temp_cs1200 + 1
				avg_hours_ot_week_csci1200[day_of_week][hour] = avg_hours_ot_week_csci1200[day_of_week][hour]+1
			elif(info[1]=="csci2600"):#principles of Software
				submission_temp_cs2600 = submission_temp_cs2600 + 1
			elif(info[1]=="csci1100"):#cs1
				submission_temp_cs1100 = submission_temp_cs1100 + 1
			elif(info[1]== "csci4430"):#prog lang
				submission_temp_cs4430 = submission_temp_cs4430 + 1
			elif(info[1]== "csci2200"): #focs
				submission_temp_cs2200 = submission_temp_cs2200 + 1
			elif(info[1]== "csci4380"): #opsys
				submission_temp_cs4380 = submission_temp_cs4380 + 1

			avg_hours_ot_week[day_of_week][hour] = avg_hours_ot_week[day_of_week][hour]+1
			submissions_per_hour[hour] = submissions_per_hour[hour]+1
			new_day = False
	...
	if(full_week==7):
		...
		submission_temp_cs1200 =submission_temp_cs1200/2
		submission_temp_cs2600 = submission_temp_cs2600/2
		submission_temp_cs1100 = submission_temp_cs1100/2
		submission_temp_cs4430 = submission_temp_cs4430/2
		submission_temp_cs4380 = submission_temp_cs4380/2
...
num_of_sumbissions = num_of_sumbissions/2;
...
for day in range(7):
	...
	for hour in range(24):
		...
		if(avg_hours_ot_week[day][hour] != 0):
			avg_waittime_ot_week[day][hour] = float(float(avg_waittime_ot_week[day][hour])/float(avg_hours_ot_week[day][hour]))
			avg_gradingtime_ot_week[day][hour] = float(float(avg_gradingtime_ot_week[day][hour])/float(avg_hours_ot_week[day][hour]))
