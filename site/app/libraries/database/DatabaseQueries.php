...
ORDER BY {$u_or_t}.{$section_key}
    ...
    public function getVerifiedComponentsCountByGradingSections(string $g_id, array $sections, string $section_key, bool $is_team): array {
        ...
        if(! in_array($section_key, array("registration_section", "rotating_section"))){
            return [];
        }
        ...
        $this->course_db->query(
            ...
            WHERE
                gd.g_id = ?
                AND CAST ($unit.$section_key AS TEXT) = ANY (ARRAY[" . $this->createParameterList(sizeof($sections)) . "])
