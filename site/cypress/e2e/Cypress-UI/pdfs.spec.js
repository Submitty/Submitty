function switch_settings(gradeable_id) {
    ...
    cy.get(
        'input[data-testname="grader_assignment_method"][value="1"]'
    ).check();
}
...
function revert_settings(gradeable_id, setting) {
    ...
    cy.get("#page_3_nav").click();
    cy.get(
        `input[data-testname="grader_assignment_method"][id=${setting}`
    ).check();
}
...
function no_pdf_access(gradeable_id) {
    ...
    cy.get('[data-testid="popup-message"]')
        .should("be.visible")
        .and("contain.text", "You do not have permission");
}
