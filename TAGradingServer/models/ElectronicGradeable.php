...
SELECT g_title, gd_overall_comment, g_grade_start_date, eg.* FROM electronic_gradeable AS eg 
    ...
    INNER JOIN gradeable_data AS gd ON gd.g_id=g.g_id 
        ...
        if (empty($this->eg_details)) {
            ...
            Database::query("INSERT INTO gradeable_data(g_id,gd_user_id,gd_grader_id,gd_overall_comment, gd_status,gd_late_days_used,gd_active_version, user_viewed_date ) VALUES(?,?,?,?,?,?,?, NULL)", $params);
