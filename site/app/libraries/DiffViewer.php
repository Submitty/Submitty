    ...
    private function getDisplay($lines, $type = self::EXPECTED, $option = self::SPECIAL_CHARS_ORIGINAL) {
        ...
        if (isset($this->add[$type]) && count($this->add[$type]) > 0) {
            if (array_key_exists(-1, $this->add[$type])) {
                ...
                for ($k = 0; $k < $num_blanks; $k++) {
                    $html .= "\t<div class='row bad{$type === "expected" ? "-expected" : ""}'><div class='empty_line'>&nbsp;</div></div>\n" ;
                }
                $html .= "\t</div>\n";
            }
        }
        ...
        for ($i = 0; $i < count($lines); $i++) {
            ...
            if (isset($this->diff[$type][$i])) {
                $html .= "\t<div class='bad{$type === "expected" ? "-expected" : ""}'>";
            }
            else {
                $html .= "\t<div>";
            }
            ...
            if (isset($this->diff[$type][$i])) {
                ...
                foreach ($this->diff[$type][$i] as $diff) {
                    ...
                    if ($option == self::SPECIAL_CHARS_ORIGINAL) {
                        $html .= $html_orig;
                        $html .= "<span class='highlight-char{$type === "expected" ? "-expected" : ""}'>" . $html_orig_error . "</span>";
                    }
                    elseif ($option == self::SPECIAL_CHARS_UNICODE) {
                        ...
                        $html .= $html_no_empty;
                        $html .= "<span class='highlight-char{$type === "expected" ? "-expected" : ""}'>" . $html_no_empty_error . "</span>";
                    }
            ...
            if (isset($this->add[$type][$i])) {
                ...
                for ($k = 0; $k < $this->add[$type][$i]; $k++) {
                    $html .= "\t<div class='bad{$type === "expected" ? "-expected" : ""}'><td class='empty_line' colspan='2'>&nbsp;</td></div>\n";
                }
                if ($start === null) {
                    $html .= "\t</div>\n";
                }
