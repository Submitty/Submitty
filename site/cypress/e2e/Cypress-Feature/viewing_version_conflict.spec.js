...
describe('Test revolving around the student side and whether or not they should see a version conflict', () => {
    it('Student should have a version conflict', () => {
        ...
        cy.get('[data-testid="version-conflict-version-box"]').contains('Note: The version you have selected to be graded is not the version graded by the instructor/TAs.');
        cy.get('[data-testid="version-conflict-version-box"]').contains('If the graded version does not match your selected version, a zero will be recorded in the gradebook.');

        cy.get('[data-testid="version-conflict-ta"]').should('exist');
        cy.get('[data-testid="version-conflict-ta"]').contains('has been detected in your submission. A grade of zero will be recorded in the gradebook. Please resolve as necessary.');
        ...
        cy.get('[data-testid="ta-grade-results"]').each(($el, index) => {
            cy.wrap($el).contains('For Version #2');
        });
    });
