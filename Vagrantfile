...
def mount_folders(config, mount_options, type = nil, host = '10.0.2.2')
 # ideally we would use submitty_daemon or something as the owner/group, but since that user doesn't exist
  ...
  optional_repos = %w(AnalysisTools AnalysisToolsTS Lichen RainbowGrades Tutorial CrashCourseCPPSyntax IntroQuantumComputing CrashCourseQuantumComputing LichenTestData DockerImages DockerImagesRPI)
