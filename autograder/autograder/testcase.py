...
class Testcase:
    ...
    def __init__(
        ...
        solution_containers = testcase_info.get("solution_containers", None)
    ...
    def execute(self):
        ...
        self.secure_environment.log_container(
            f"TESTCASE {self.testcase_id} END", "", "", timer() - testcase_start
        )
        self.secure_environment.log_container("")

        # force cleanup docker networks at the end of each test case
        docker_cleanup_output = os.popen(
            "sudo /usr/local/submitty/sbin/docker_cleanup.sh " + self.untrusted_user
        ).read()
    ...
    def _run_execution(self):
        ...
        with open(os.path.join(self.secure_environment.tmp_logs, "runner_log.txt"), "a") as logfile:
            ...
            display_line = (
                [] if display_sys_variable == "" else ["--display", str(display_sys_variable)]
            )
            ...
            try:
                ...
                success = self.secure_environment.execute(
                    self.untrusted_user, "my_runner.out", arguments, logfile
                )
            except Exception:
                self.secure_environment.log_message(
                    "ERROR thrown by main runner. See traces entry for more details."
                )
    ...
    def _run_compilation(self):
        ...
        with open(comp_log_directory, "a") as logfile:
            ...
            try:
                # Execute this testcase using our secure environment.
                success = self.secure_environment.execute(
                    self.untrusted_user, "my_compile.out", arguments, logfile
                )
            except Exception:
                success = -1
                self.secure_environment.log_message(
                    "ERROR thrown by main compile. See traces entry for more details."
                )
                self.secure_environment.log_stack_trace(traceback.format_exc())
            finally:
                # Lock down permissions on our execution folder.
    ...
    def generate_random_inputs(self):
        ...
        with open(log_directory, "a") as logfile:
            ...
            try:
                ...
                success = -1
                self.secure_environment.log_message(
                    "ERROR thrown by input generator. See traces entry for more details."
                )
                self.secure_environment.log_stack_trace(traceback.format_exc())
            finally:
    ...
    def generate_random_outputs(self):
        ...
        with open(log_dir, "a") as logfile:
            ...
            try:
                ...
                success = -1
                self.secure_environment.log_message(
                    "ERROR thrown by output generator. See traces entry for more details."
                )
                self.secure_environment.log_stack_trace(traceback.format_exc())
            finally:
