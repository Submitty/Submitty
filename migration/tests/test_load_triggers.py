...
class TestLoadTriggers(unittest.TestCase):
    def setUp(self):
        ...
        for i in range(0, len(self.data_master)):
            with (migrator.TRIGGERS_PATH / "master" / "test_fn_master_{}.sql".format(i)).open(
                "w"
            ) as file:
        ...
        for i in range(0, len(self.data_course)):
            with (migrator.TRIGGERS_PATH / "course" / "test_fn_course_{}.sql".format(i)).open(
                "w"
            ) as file:
        ...
        for show_output in (True, False):
            ...
            self.assertEqual(
                "Loading trigger functions to master...\n  {}\nDONE\n".format(
                    "\n  ".join(
                        ["test_fn_master_{}".format(self.data_master.index(n)) for n in data]
                    )
                ),
                sys.stdout.getvalue(),
            )
    ...
    def test_course(self):
        ...
        for show_output in (True, False):
            ...
            mock_instances[0].execute.return_value.mappings.return_value.all.return_value = [
                {"term": "s{}".format(i), "course": "c{}".format(i)} for i in range(0, num_courses)
            ]
            ...
            for i in range(0, num_courses):
                self.assertEqual(
                    "submitty_s{0}_c{0}".format(i), mock_class.call_args_list[i + 1][0][0]["dbname"]
                )
            ...
            if show_output:
                for coursedb in coursedbs:
                    ...
                    course_logs += (
                        "Loading trigger functions to s{0}.c{0}...\n  {1}\nDONE\n".format(
                            i,
                            "\n  ".join(
                                [
                                    "test_fn_course_{}".format(self.data_course.index(n))
                                    for n in data
                                ]
                            ),
                        )
    ...
    def test_no_trigger_files(self):
        ...
        for show_output in (True, False):
            ...
            self.assertEqual(
                "Loading trigger functions to master...DONE\n"
        ...
        for show_output in (True, False):
            with patch.object(migrator.db, "Database") as mock_class:
                mock_class.side_effect = OperationalError(
                    None, None, "first " + ("a" if show_output else "b") + "\nsecond"
                )
    ...
    def test_course_db_fail(self):
        ...
        for show_output in (True, False):
            ...
            masterdb.execute.return_value.mappings.return_value.all.return_value = [
                {"term": "my_semester_1", "course": "my_course_1"},
                {"term": "my_semester_2", "course": "my_course_2"},
            ]
            ...
            with patch.object(migrator.db, "Database") as mock_class:
                mock_class.side_effect = [
                    ...
                    OperationalError(
                        None, None, "third " + ("a" if show_output else "b") + "\nfourth"
                    ),
                ]
                if show_output:
                    migrator.main.load_triggers(args)
